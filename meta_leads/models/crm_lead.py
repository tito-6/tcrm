...
class CrmLead(models.Model):
    ...
    # ===================================================================
    # Idempotency fields (Fall-back protection)
    # ===================================================================
    tcrm_idempotency_scope = fields.Char(string='Idempotency Scope', readonly=True, index=True)
    tcrm_idempotency_key = fields.Char(string='Idempotency Key', readonly=True, index=True)
    tcrm_idempotency_payload_sha256 = fields.Char(string='Idempotency Payload SHA-256', readonly=True)
    tcrm_external_submission_id = fields.Char(string='External Submission ID', readonly=True)

    _sql_constraints = [
        ('unique_tcrm_idempotency', 'unique(tcrm_idempotency_scope, tcrm_idempotency_key)', 'Idempotency key must be unique per scope.')
    ]
    ...
    # ===================================================================
    def write(self, vals):
        """Override write to detect stage changes and fire Meta CAPI events."""
        # Capture old stage for comparison
        old_stages = {}
        if 'stage_id' in vals:
            for lead in self:
        ...
        # If stage_id changed, check whether we need to fire CAPI events
        if 'stage_id' in vals:
            for lead in self:
    ...
    def _check_and_fire_capi_event(self, lead):
        ...
        if not pixel_id or not access_token:
            _logger.warning(
                "Meta CAPI credentials not configured — skipping event for lead %s",
                lead.id,
            )
            return
        ...
        try:
            ...
            event_source_url = lead.meta_event_source_url or "https://akod.tech"

            # --- Check for QUALIFIED stage → Lead event ---
            if self._is_qualified_stage(stage_name) and not lead.meta_capi_lead_event_sent:
                result = capi.send_lead_event(lead, event_source_url=event_source_url)
                if result.get("success"):
                    lead.sudo().write({
                        'meta_capi_lead_event_sent': True,
                        'meta_capi_last_event': 'Lead',
                        'meta_capi_last_event_time': fields.Datetime.now(),
                        'meta_capi_event_log': self._append_event_log(
                            lead.meta_capi_event_log,
                            'Lead',
                            result,
                        ),
                    })
                    _logger.info(
                        "✅ Meta CAPI Lead event sent for lead %s (stage: %s)",
                        lead.id, lead.stage_id.name,
                    )
                else:
                    _logger.error(
                        "❌ Meta CAPI Lead event FAILED for lead %s: %s",
                        lead.id, result.get("error"),
                    )

            # --- Check for WON stage → Purchase event ---
            elif self._is_won_stage(stage_name) and not lead.meta_capi_purchase_event_sent:
                ...
                if result.get("success"):
                    lead.sudo().write({
                        'meta_capi_purchase_event_sent': True,
                        'meta_capi_last_event': 'Purchase',
                        'meta_capi_last_event_time': fields.Datetime.now(),
                        'meta_capi_event_log': self._append_event_log(
                            lead.meta_capi_event_log,
                            'Purchase',
                    ...
                    _logger.info(
                        "✅ Meta CAPI Purchase event sent for lead %s (stage: %s, value: %s TRY)",
                        lead.id, lead.stage_id.name, value,
                    )
                else:
                    _logger.error(
                        "❌ Meta CAPI Purchase event FAILED for lead %s: %s",
                        lead.id, result.get("error"),
                    )
