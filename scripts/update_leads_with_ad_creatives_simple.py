#!/usr/bin/env python3
"""
Update existing leads with Ad Creative Preview content - Simple Version
Fetches basic ad creative data from Facebook and formats it nicely
"""
import odoorpc
import requests
import json
...
def fetch_ad_basic_data(ad_id):
    ...
    try:
        ...
        if ad_response.status_code == 200:
            ...
            print(f"      ⚠️ Error fetching ad: {ad_response.status_code} - {ad_response.text}")
            return None
...
def update_leads_with_ad_creatives():
    ...
    if updated_count > 0:
        ...
        print(f"   🎨 Visual ad content layout")
        print(f"   📱 Ad name and information")
        print(f"   💬 Ad text content")
        print(f"   🎯 Call-to-action buttons")
        print(f"   📊 Ad status and details")
