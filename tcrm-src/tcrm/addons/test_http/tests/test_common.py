...
import tcrm
from tcrm.http import Session
from tcrm.addons.base.tests.common import HttpCaseWithUserDemo
from tcrm.tools import config, reset_cached_properties
from tcrm.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(tcrm.http.Application, 'session_store', session_store)
        cls.classPatch(tcrm.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(tcrm.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        tcrm.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('tcrm.http.db_list') as db_list, \
             patch('tcrm.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('tcrm.http.db_list') as db_list, \
             patch('tcrm.http.db_filter') as db_filter, \
             patch('tcrm.http.Registry') as Registry:
