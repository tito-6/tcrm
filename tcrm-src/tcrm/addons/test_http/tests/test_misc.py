...
import tcrm
from tcrm.http import content_disposition, root
from tcrm.tests import tagged
from tcrm.tests.common import HOST, BaseCase, get_db_name, new_test_user
from tcrm.tools import config, file_path, mute_logger, parse_version

from .test_common import TestHttpBase
from tcrm.addons import test_http
from tcrm.addons.test_http.controllers import CT_JSON
from tcrm.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = tcrm.tools.misc.DotDict(db=False)
            return tcrm.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'tcrm.com/{uri}', host='tcrm.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://tcrm.com/{uri}', host='tcrm.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'tcrm.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://tcrm.com/{uri}'), "No host allowed")
    ...
    def test_misc4_rpc_qweb(self):
        ...
        for method in (self.db_url_open, self.nodb_url_open):
            with self.subTest(method=method.__name__):
                with mute_logger('tcrm.addons.rpc.controllers.jsonrpc'):
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'tcrm.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(tcrm.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc8_concurrency_error(self):
        with (
            self.assertLogs('tcrm.service.model') as log_catcher,
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(tcrm.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = tcrm.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            tcrm.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
