...
from tcrm.tests.common import RecordCapturer, TransactionCase
from tcrm.tools import mute_logger

from tcrm.addons.base.models.ir_cron import (
...
class TestIrCron(TransactionCase, CronMixinCase):
        ...
        for cb, curr_failures, trigger, call_count, done_count, fail_count, active in CASES:
            with self.subTest(cb=cb, failure=curr_failures), closing(self.cr.savepoint()):
                ...
                with self.enter_registry_test_mode():
                    ...
                    with mute_logger('tcrm.addons.base.models.ir_cron'),\
    ...
    def test_cron_failed_increase(self):
        ...
        with self.enter_registry_test_mode():
            with (
                ...
                mute_logger('tcrm.addons.base.models.ir_cron'),
                self.registry.cursor() as cr,
            ):
                self.registry['ir.cron']._process_job(
                    cr,
    ...
    def test_cron_timeout_failure(self):
        ...
        self.env.flush_all()
        with self.enter_registry_test_mode(), mute_logger('tcrm.addons.base.models.ir_cron'), self.registry.cursor() as cr:
            self.registry['ir.cron']._process_job(
                cr,
