...
from tcrm.cli.command import commands, load_addons_commands, load_internal_commands
from tcrm.tests import BaseCase
from tcrm.tools import config, file_path
...
class TestCommand(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.tcrm_bin = Path(__file__).parents[4].resolve() / 'tcrm-bin'
        addons_path = config.format('addons_path', config['addons_path'])
        cls.run_args = (sys.executable, cls.tcrm_bin, f'--addons-path={addons_path}')
    ...
    def test_docstring(self):
        ...
        for name, cmd in commands.items():
            self.assertTrue(cmd.__doc__,
                msg=f"Command {name} needs a docstring to be displayed with 'tcrm-bin help'")
            self.assertFalse('\n' in cmd.__doc__ or len(cmd.__doc__) > 120,
                msg=f"Command {name}'s docstring format is invalid for 'tcrm-bin help'")
        ...
        for name in ('bonbon', 'café'):
            with self.subTest(name):
                ...
                self.assertEqual(
                    ...
                    "Use 'tcrm-bin --help' to see the list of available commands."
                )
    ...
    def test_upgrade_code_standalone(self):
        from tcrm.cli import upgrade_code  # noqa: PLC0415
        proc = sp.run(
