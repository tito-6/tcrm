...
@tcrm.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(tcrm.tests.TransactionCase):
...
class TestReportsRenderingCommon(tcrm.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        tcrm.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@tcrm.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@tcrm.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(tcrm.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
