...
import tcrm
from tcrm.tests import TransactionCase
from tcrm.tools import file_open, file_open_temporary_directory, file_path
from tcrm.tools.config import configmanager

EMPTY_CONFIG_PATH = file_path('base/tests/config/empty.conf')
PROJECT_PATH = tcrm.tools.config.root_path.removesuffix('/tcrm')
DEFAULT_DATADIR = tcrm.tools.config._default_options['data_dir']

MISSING_HTTP_INTERFACE = """\
WARNING:tcrm.tools.config:missing --http-interface/http_interface, \
...
class TestConfigManager(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        patcher = patch.dict('os.environ', {'tcrm_RC': EMPTY_CONFIG_PATH}, clear=True)
        patcher.start()
    ...
    def test_01_default_config(self):
        self.assertConfigEqual({
            ...
            'proxy_access_token': '',
            'publisher_warranty_url': 'http://services.tcrm.com/publisher-warranty/',
            'reportgz': False,
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('tcrm.tools.config', 'WARNING') as capture:
        ...
        self.assertEqual(capture.output, [
            "WARNING:tcrm.tools.config:option addons_path, no such directory '/tmp/tcrm', skipped",
            "WARNING:tcrm.tools.config:option upgrade_path, no such directory '/tmp/upgrade', skipped",
            "WARNING:tcrm.tools.config:option pre_upgrade_scripts, no such file '/tmp/pre-custom.py', skipped",
        ])
    ...
    def test_04_tcrm16_config_file(self):
        # test that loading the tcrm 16.0 generated default config works
        ...
        with (
            ...
            self.assertLogs('tcrm.tools.config', 'WARNING') as capture_warn,
        ):
        ...
        def missing(*options):
            return [
                f"WARNING:tcrm.tools.config:unknown option '{option}' in "
        ...
        def falsy(*options):
            return [
                f"WARNING:tcrm.tools.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:tcrm.tools.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        for args, stop_after_init in [
            ...
            with self.subTest(args=args):
                if any('--test' in arg for arg in args):
                    ...
                    self.assertEqual(capture.output, [
                        "WARNING:tcrm.tools.config:Empty -d/--database/db_name, tests won't run",
                    ])
                else:
                    _, options = self.parse_reset(args)
