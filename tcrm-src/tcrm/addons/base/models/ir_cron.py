...
from tcrm import api, fields, models, sql_db
...
from tcrm.tools.constants import GC_UNLINK_LIMIT
...
BASE_VERSION = (Manifest.for_addon('base', display_warning=False) or {}).get('version', '1.3')
...
tcrm_NOTIFY_FUNCTION = os.getenv('tcrm_NOTIFY_FUNCTION', 'pg_notify')
...
class IrCron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('tcrm_NOTIFY_CRON_CHANGES'):
    ...
    def _check_modules_state(cr, jobs):
        ...
        # reset_module_states.
        from tcrm.modules.loading import reset_modules_state  # noqa: PLC0415
    ...
    def write(self, vals):
        ...
        if ('nextcall' in vals or vals.get('active')) and os.getenv('tcrm_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list: list[datetime]):
        ...
        if min(at_list) <= now or os.getenv('tcrm_NOTIFY_CRON_CHANGES'):
            self.env.cr.postcommit.add(self._notifydb)
        return triggers
    ...
    def _notifydb(self):
        ...
        The tcrm_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        IrCron modification and on trigger creation (regardless of call_at)
        """
        with sql_db.db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(tcrm_NOTIFY_FUNCTION), self.env.cr.dbname))
