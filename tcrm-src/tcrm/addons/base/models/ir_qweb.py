...
QWeb is the primary templating engine used by tcrm. It is an XML templating
...
<https://www.tcrm.com/documentation/master/developer/reference/frontend/qweb.html>`
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
**Values**: name of the allowed tcrm user group, or preceded by ``!`` for
prohibited groups

The generated code uses ``has_group`` tcrm method from ``res.users`` model.
...
from tcrm.modules.registry import _REGISTRY_CACHES
from tcrm.tools import config, safe_eval, OrderedSet, frozendict, json
from tcrm.tools.constants import SUPPORTED_DEBUGGER, EXTERNAL_ASSET
from tcrm.tools.safe_eval import assert_valid_codeobj, _BUILTINS, to_opcodes, _EXPR_OPCODES, _BLACKLIST
from tcrm.tools.lru import LRU
from tcrm.tools.misc import str2bool, file_open, file_path
from tcrm.tools.image import image_data_uri, FILETYPE_BASE64_MAGICWORD
from tcrm.tools.profiler import ExecutionContext
from tcrm.tools.translate import FORMAT_REGEX
from tcrm.http import request
from tcrm.tools.profiler import QwebTracker
from tcrm.exceptions import UserError, MissingError

from tcrm.addons.base.models.assetsbundle import AssetsBundle
from tcrm.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS, TEMPLATE_EXTENSIONS
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render_iterall(self, view_ref, method, values, directive='render') -> Iterator[str]:
        ...
        try:
            ...
            if hasattr(error, 'qweb'):
                ...
                # If is not an tcrm Exception check if the current error is raise from
    ...
    def _debug_trace(self, debugger, values):
        ...
        if not debugger:
            ...
            warnings.warn(
                ...
                "since tcrm 17.0, keep the value empty and configure the "
    ...
    def _link_to_node(self, path, defer_load=False, lazy_load=False, media=None):
        ...
        if is_js:
            ...
            if is_asset_bundle:
                attributes['onerror'] = "__tcrmAssetError=1"
