...
from tcrm.exceptions import AccessDenied, AccessError, UserError, ValidationError
from tcrm.fields import Command, Domain
from tcrm.http import request, DEFAULT_LANG
from tcrm.tools import email_domain_extract, is_html_empty, frozendict, reset_cached_properties, SQL
...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, credential, env):
        ...
        * catch :class:`~tcrm.exceptions.AccessDenied` and perform their
          own checking
        * (re)raise :class:`~tcrm.exceptions.AccessDenied` if the
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by tcrm (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: tcrm.exceptions.AccessDenied when old password is wrong
        :raise: tcrm.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this tcrm portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your tcrm is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "tcrm in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.tcrm.com/documentation/latest/administration/install/deploy.html#https for details.",
                    source
                )
