...
if typing.TYPE_CHECKING:
    ...
    from tcrm.orm.environments import Transaction
...
class Cursor(BaseCursor):
    ...
    def __init__(self, pool: ConnectionPool, dbname: str, dsn: dict):
        ...
        if os.getenv('tcrm_FAKETIME_TEST_MODE') and self.dbname in tools.config['db_name']:
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # tcrm Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri: str, readonly=False) -> tuple[str, dict]:
    ...
    if 'tcrm_PGAPPNAME' in os.environ:
        warnings.warn("Since 19.0, use PGAPPNAME instead of tcrm_PGAPPNAME", DeprecationWarning)
        app_name = os.environ['tcrm_PGAPPNAME']
...
def db_connect(to: str, allow_uri=False, readonly=False) -> Connection:
    ...
    maxconn = (tools.config['db_maxconn_gevent'] if hasattr(tcrm, 'evented') and tcrm.evented else 0) or tools.config['db_maxconn']
...
def close_db(db_name: str) -> None:
    """ You might want to call tcrm.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
