...
from tcrm.tools import config, topological_sort, unique, profiler
from tcrm.modules.registry import Registry
from tcrm.netsvc import init_logger
from tcrm.tests import standalone_tests
import tcrm.tests.loader

_logger = logging.getLogger('tcrm.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of tcrm modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra tcrm modules")
...
def test_standalone(args):
    ...
    tcrm.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        tcrm.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        tcrm.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'tcrm.modules.loading': {'level': 'CRITICAL'},
            'tcrm.sql_db': {'level': 'CRITICAL'},
            'tcrm.models.unlink': {'level': 'WARNING'},
            'tcrm.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
