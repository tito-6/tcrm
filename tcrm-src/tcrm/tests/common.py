# -*- coding: utf-8 -*-
"""
The module :mod:`tcrm.tests.common` provides unittest test cases and a few
...
import tcrm.addons.base
import tcrm.cli
import tcrm.http
import tcrm.models
import tcrm.orm.registry
from tcrm import api
from tcrm.exceptions import AccessError
from tcrm.fields import Command
from tcrm.modules.registry import Registry, DummyRLock
from tcrm.service import security
from tcrm.sql_db import Cursor, Savepoint
from tcrm.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from tcrm.tools.mail import single_email_re
from tcrm.tools.misc import find_in_path, lower_logging
from tcrm.tools.xml_utils import _validate_xml
from tcrm.addons.base.models import ir_actions_report

from . import case, test_cursor
from .result import tcrmTestResult
...
if tcrm.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: tcrm.tests.common.Form is deprecated, use tcrm.tests.Form",
...
# The tcrm library is supposed already configured.
HOST = '127.0.0.1'
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = tcrm.modules.module.current_test.canonical_tag
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding tcrm module name.
    """
    def register(func):
        # register func by tcrm module name
        if func.__module__.startswith('tcrm.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most tcrm operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for tcrm-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('tcrm.addons.'):
            if getattr(cls, 'test_tags', None) is None:
                cls.test_tags = {'standard', 'at_install'}
            cls.test_module = cls.__module__.split('.')[2]

    longMessage = True      # more verbose error message by default: https://www.tcrm.com/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('tcrm_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def run(self, result: tcrmTestResult) -> None:
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~tcrm.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(tcrm.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            tcrm.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = tcrm.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('tcrm.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/tcrm/addons/" in filename:
                        filename = filename.rsplit("/tcrm/addons/", 1)[1]
                    if count > expected:
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case tcrm.fields.Many2one():
                        record_value = record_value.id
                    case tcrm.fields.One2many() | tcrm.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case tcrm.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case tcrm.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def assertCanOpenTestCursor(self):
        ...
        if tcrm.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {tcrm.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = tcrm.http.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if tcrm.tools.config['test_tags'] and 'is_query_count' in tcrm.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | tcrm.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(tcrm.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('tcrm.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(tcrm.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_tcrm')

        if scs := tcrm.tools.config['screencasts']:
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way tcrm widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    touch_enabled = False
    session: tcrm.http.Session = None
    ...
    @classmethod
    def http_port(cls):
        if tcrm.service.server.server is None:
            return None
        return tcrm.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('tcrm.service.http.request.')]
        ...
        if request_threads:
            ...
            tcrm.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        tcrm.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            tcrm.http.root.session_store.delete(self.session)

        self.session = session = tcrm.http.root.session_store.new()
        session.update(tcrm.http.get_default_session(), db=get_db_name())
        session.context['lang'] = tcrm.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('tcrm.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        tcrm.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from tcrm.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from tcrm.addons.bus.models.bus import BusBus  # noqa: PLC0415
            ...
            cpu_throttling_os = os.environ.get('tcrm_BROWSER_CPU_THROTTLING')  # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("tcrm_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"tcrm.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"tcrm.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('tcrm.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if tcrm.tools.config['test_tags'] and 'is_tour' in tcrm.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from tcrm.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in tcrm test suites
