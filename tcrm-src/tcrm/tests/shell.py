...
TEST_MODULE_NAME_PATTERN = re.compile(r'^tcrm\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if tcrm.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in tcrm shell')
        return

    if tcrm.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from tcrm.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    tcrm.tools.config['test_tags'] = test_tags
    tcrm.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    tcrm.tools.config['test_enable'] = None
    tcrm.tools.config['test_tags'] = None
