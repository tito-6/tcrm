...
""" Initialize the database for module management and tcrm installation. """
...
if typing.TYPE_CHECKING:
    from tcrm.sql_db import BaseCursor, Cursor
...
def is_initialized(cr: Cursor) -> bool:
    ...
    """
    return tcrm.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr: Cursor) -> None:
    ...
    """
    try:
        f = tcrm.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise OSError(m)

    with tcrm.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for info in tcrm.modules.Manifest.all_addon_manifests():
    ...
    from tcrm.tools import config  # noqa: PLC0415
...
def has_trigram(cr: BaseCursor) -> bool:
    ...
    pg_trgm module but any similar function will be picked by tcrm.
