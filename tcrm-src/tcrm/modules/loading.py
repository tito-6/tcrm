...
import tcrm.sql_db
import tcrm.tools.sql
import tcrm.tools.translate
from tcrm import api, tools
from tcrm.tools import OrderedSet
from tcrm.tools.convert import convert_file, IdRef, ConvertMode as LoadMode
...
def load_data(env: Environment, idref: IdRef, mode: LoadMode, kind: LoadKind, package: ModuleNode) -> bool:
    ...
    for k in keys:
        if k == 'init_xml' and package.manifest[k]:
            _logger.warning("module %s: key 'init_xml' is deprecated in tcrm 19.", package.name)
...
def load_module_graph(
    ...
    report: tcrmTestResult | None = None,
    ...
    assert isinstance(env.cr, tcrm.sql_db.Cursor), "Need for a real Cursor to load modules"
    ...
    t0 = time.time()
    loading_extra_query_count = tcrm.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = tcrm.sql_db.sql_counter
        ...
        if update_operation == 'install':
            py_module = sys.modules['tcrm.addons.%s' % (module_name,)]
        ...
        if tools.config['test_enable'] and (update_operation or not update_from_config):
            ...
            if suite.countTestCases():
                ...
                # Python tests
                tests_t0, tests_q0 = time.time(), tcrm.sql_db.sql_counter
                ...
                test_queries = tcrm.sql_db.sql_counter - tests_q0

                # tests may have reset the environment
                module = env['ir.module.module'].browse(module_id)


        extra_queries = tcrm.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   tcrm.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
    ...
    with registry.cursor() as cr:
        ...
        if update_module and upgrade_modules:
            for pyfile in tools.config['pre_upgrade_scripts']:
                tcrm.modules.migration.exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')
        ...
        if update_module and tools.sql.table_exists(cr, 'ir_model_fields'):
            ...
            if tcrm.tools.sql.column_exists(cr, 'ir_model_fields', 'company_dependent'):
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['tcrm.addons.%s' % (pkg.name,)]
