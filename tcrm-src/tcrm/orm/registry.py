...
from tcrm.tools.func import locked, reset_cached_properties
from tcrm.tools.lru import LRU
from tcrm.tools.misc import Collector, format_frame
...
if typing.TYPE_CHECKING:
    ...
    from tcrm.fields import Field
    from tcrm.models import BaseModel
    from tcrm.sql_db import BaseCursor, Connection, Cursor
    from tcrm.modules import module_graph


_logger = logging.getLogger('tcrm.registry')
_schema = logging.getLogger('tcrm.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from tcrm.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            # This should be a method on Registry
            from tcrm.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from tcrm.tests.result import tcrmTestResult  # noqa: PLC0415
            self._assertion_report: tcrmTestResult | None = tcrmTestResult()
        else:
        ...
        for indexname, tablename, field in expected:
            ...
            if index and indexname not in existing:
                ...
                if index == 'trigram':
                    ...
                    from tcrm.modules.db import FunctionStatus  # noqa: PLC0415
