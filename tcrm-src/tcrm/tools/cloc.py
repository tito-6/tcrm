...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in tcrm.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_path = {
            ...
            if (m := tcrm.modules.Manifest.for_addon(name, display_warning=False))
        }
        ...
        for module_name in module_list:
            manifest = tcrm.modules.Manifest.for_addon(module_name)
    ...
    def count_customization(self, env):
        ...
        # the module should be tcrm/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = tcrm.modules.registry.Registry(database)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="tcrm cloc", lines="Line", other="Other", code="Code")
        s += hr
