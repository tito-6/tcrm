...
from tcrm import modules
from tcrm.tools.arabic_reshaper import reshape
from tcrm.tools.parse_version import parse_version
from tcrm.tools.misc import file_open, SENTINEL
...
def add_banner(pdf_stream, text=None, logo=False, thickness=SENTINEL):
    """ Add a banner on a PDF in the upper right corner, with tcrm's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display tcrm's logo in the banner.
    ...
    with file_open('base/static/img/main_partner-image.png', mode='rb') as f:
        tcrm_logo_file = io.BytesIO(f.read())
    tcrm_logo = Image.open(tcrm_logo_file)
    tcrm_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(tcrm_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class tcrmPdfFileReader(PdfFileReader):
...
class tcrmPdfFileWriter(PdfFileWriter):
    ...
    def embed_tcrm_attachment(self, attachment, subtype=None):
        assert attachment, "embed_tcrm_attachment cannot be called without attachment."
