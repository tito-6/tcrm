"""
This code is what let us use ES6-style modules in tcrm.
Classic tcrm modules are composed of a top-level :samp:`tcrm.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@tcrm-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** tcrm-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual tcrm one with minimal changes.
...
from tcrm.tools.misc import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom tcrm modules.
    ...
    legacy_tcrm_define = get_aliased_tcrm_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_tcrm_define, module_path, dependencies),
        partial(convert_t, url)
    ]
    for s in steps:
        content = s(content)
    if legacy_tcrm_define:
        content += legacy_tcrm_define
    return content
...
def url_to_module_path(url):
    """
    tcrm modules each have a name. (tcrm.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_tcrm_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the tcrm.define call.
    ...
    """
    return f"""tcrm.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
tcrm_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @tcrm-module                       # '@tcrm-module' statement
...
def is_tcrm_module(url, content):
    """
    Detect if the file is a native tcrm module.
    We look for a comment containing @tcrm-module.
...
def get_aliased_tcrm_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the tcrm source base being:
    ...
    > tcrm.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @tcrm-module */
        // after
        /** @tcrm-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @tcrm-module */
        // after
        /** @tcrm-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = tcrm_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\ntcrm.define(`%s`, ['%s'], function (require) {
                        return require('%s');
