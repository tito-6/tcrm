...
from tcrm import release
from tcrm.tools.func import classproperty
from . import appdirs
...
class _tcrmOption(optparse.Option):
    ...
    def __init__(self, *opts, **attrs):
        ...
        if env_name is None and is_new_option and self.file_loadable:
            ...
            self.env_name = 'tcrm_' + self.dest.upper()
...
class _FileOnlyOption(_tcrmOption):
...
class _PosixOnlyOption(_tcrmOption):
    def __init__(self, *opts, **attrs):
        if os.name != 'posix':
...
class configmanager:
    def __init__(self):
        ...
        # dictionary mapping option destination (keys in self.options) to tcrmOptions.
        self.options_index = {}
    ...
    @property
    def rcfile(self):
        self._warn("Since 19.0, use tcrm.tools.config['config'] instead", DeprecationWarning, stacklevel=2)
        return self['config']

    @rcfile.setter
    def rcfile(self, rcfile):
        self._warn(f"Since 19.0, use tcrm.tools.config['config'] = {rcfile!r} instead", DeprecationWarning, stacklevel=2)
        self._runtime_options['config'] = rcfile

    def _build_cli(self):
        tcrmOption = type('tcrmOption', (_tcrmOption,), {'config': self})
        FileOnlyOption = type('FileOnlyOption', (_FileOnlyOption, tcrmOption), {})
        PosixOnlyOption = type('PosixOnlyOption', (_PosixOnlyOption, tcrmOption), {})

        version = "%s %s" % (release.description, release.version)
        parser = optparse.OptionParser(version=version, option_class=tcrmOption)
        ...
        parser.add_option(FileOnlyOption(dest='publisher_warranty_url', my_default='http://services.tcrm.com/publisher-warranty/', file_exportable=False))
        ...
        group.add_option("-c", "--config", dest="config", type='path', file_loadable=False, env_name='tcrm_RC',
                         help="specify alternate config file")
        group.add_option("-s", "--save", action="store_true", dest="save", my_default=False, file_loadable=False,
                         help="save configuration to ~/.tcrmrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", type='path',  # sensitive default set in _load_default_options
                         help="Directory where to store tcrm data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'tcrm_tests')
        ...
        group.add_option('--log-handler', action="append", type='comma', my_default=[':INFO'], metavar="MODULE:LEVEL",
                         help='setup a handler at LEVEL for a given MODULE. An empty MODULE indicates the root logger. '
                              'This option can be repeated. Example: "tcrm.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const=("tcrm.http:DEBUG",),
                         help='shortcut for --log-handler=tcrm.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const=("tcrm.sql_db:DEBUG",),
                         help='shortcut for --log-handler=tcrm.sql_db:DEBUG')
        ...
        group.add_option("--db_app_name", dest="db_app_name", my_default="tcrm-{pid}", env_name='PGAPPNAME',
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate tcrm to another language. "
        ...
        group.add_option('--dev', dest='dev_mode', type='comma', metavar="FEATURE,...", my_default=[], file_exportable=False, env_name='tcrm_DEV',
    ...
    def _load_default_options(self):
        ...
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'tcrm.conf')
        elif os.path.isfile(rcfilepath := os.path.expanduser('~/.tcrmrc')):
            pass
        elif os.path.isfile(rcfilepath := os.path.expanduser('~/.openerp_serverrc')):
            self._warn("Since ages ago, the ~/.openerp_serverrc file has been replaced by ~/.tcrmrc", DeprecationWarning)
        else:
            rcfilepath = '~/.tcrmrc'
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes tcrm.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            tcrm.tools.config.parse_config(sys.argv[1:])
        """
        from tcrm import modules, netsvc  # noqa: PLC0415
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of tcrm 18, it's recommended to specify whether"
                    " you want tcrm to setup its own logging (or want to"
                    " handle it yourself)",
    ...
    def _load_env_options(self):
        ...
        if environ.get('OPENERP_SERVER'):
            self._warn("Since ages ago, the OPENERP_SERVER environment variable has been replaced by tcrm_RC", DeprecationWarning)

    def _load_cli_options(self, opt):
        # tcrm.cli.command.main parses the config twice, the second time
