...
"""
Miscellaneous tools used by tcrm.
"""
...
from tcrm.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from tcrm.addons.base.models.res_lang import LangData
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None, *, check_exists: bool = True) -> str:
    ...
    if not is_abs and (module := sys.modules.get(f'tcrm.addons.{file_path_split[0]}')):
        ...
        addons_paths = [*tcrm.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/tcrm/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with tcrm.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with tcrm.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('tcrm.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('tcrm.foo.bar'):
            do_suff()
    """
...
class OrderedSet(MutableSet[T], typing.Generic[T]):
    ...
    def copy(self):
        # WeakSet (Python 3.14+) iterates via self.data.copy(); plain set has this, MutableSet does not.
...
def format_date(
    ...
    if not value:
        return ''
    from tcrm.fields import Datetime  # noqa: PLC0415
...
def limited_field_access_token(record, field_name, timestamp=None, *, scope):
    ...
    :type record: class:`tcrm.models.Model`
