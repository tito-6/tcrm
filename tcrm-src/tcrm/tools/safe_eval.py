...
_CONST_OPCODES = set(to_opcodes([
    ...
    # 3.13
    'TO_BOOL',
    # 3.14 small int literals in tuples/lists (e.g. [(4, ref(...))])
    'LOAD_SMALL_INT',
    # 3.14 control-flow bookkeeping opcode seen in simple expressions
    'NOT_TAKEN',
])) - _BLACKLIST
...
_BUBBLEUP_EXCEPTIONS = (
    tcrm.exceptions.UserError,
    tcrm.exceptions.RedirectWarning,
...
`tcrm.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `tcrm.tools.safe_eval`.
""")
    return d
...
from tcrm._monkeypatches.pytz import patch_module as patch_pytz  # noqa: E402, F401
patch_pytz()
