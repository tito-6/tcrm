...
r"""\
tcrm HTTP layer / WSGI application
...
:class:`~tcrm.http.Controller`: class and its companion the
:func:`~tcrm.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an tcrm http request. The tcrm http request is
...
import tcrm.init  # import first for core setup
...
if geoip2:
    ...
    class _EmptyGeoIPReader:
        def city(self, ip):
            raise geoip2.errors.AddressNotFoundError(f"GeoIP database unavailable for {ip}")
...
tcrm URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.tcrm.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through tcrm via py-QWeb form, embed a CSRF
...
def _not_found_nodb_body():
    default_dbs = config['db_name']
    login_href = '/web/login?db=%s' % default_dbs[0] if default_dbs and len(default_dbs) == 1 else '/web/login'
    return f"""\
<!DOCTYPE html>
<title>404 Not Found</title>
<h1>Not Found</h1>
<p>No database is selected and the requested URL was not found in the server-wide controllers.</p>
<p>Please verify the hostname, <a href={login_href!r}>login</a> and try again.</p>

<!-- Alternatively, use the X-tcrm-Database header. -->
"""
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~tcrm.service.db.list_dbs`:
    ...
    """
    try:
        dbs = tcrm.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, tcrm will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': tcrm.service.common.dispatch,
        'db': tcrm.service.db.dispatch,
        'object': tcrm.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~tcrm.tools.file_path`
        :param filter_ext: See :func:`~tcrm.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`tcrm.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~tcrm.http.Controller` can use the :func:`~tcrm.http.route`:
    ...
    :func:`~tcrm.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(tcrm.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['tcrm', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~tcrm.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['tcrm', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are tcrm addons.
        """
        # Controllers defined outside of tcrm addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of tcrm addons can be extended in
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~tcrm.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> tcrm_ip = socket.gethostbyname('tcrm.com')
        >>> GeoIP(tcrm_ip).country.iso_code
        'FR'
    """
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~tcrm.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # tcrm.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        header_dbname = self.httprequest.headers.get('X-tcrm-Database')
        if session.db and db_filter([session.db], host=host):
            ...
            elif len(all_dbs) == 0 and config['db_name'] and len(config['db_name']) == 1:
                # Single default db in config but not in list (e.g. not created yet): use it
                # so request is served with db and user gets a clear error or login.
                dbname = config['db_name'][0]
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~tcrm.addons.base.models.res_users.ResUsers>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~tcrm.http.Response`
        """
    ...
    def _serve_nodb(self):
        ...
        try:
            ...
            try:
                ...
                # When no route matches, redirect / or /web to login with default db so user
                # does not see "No database is selected". Do not redirect /web/login to avoid loop
                # when server-wide modules (e.g. web) failed to load.
                path = (self.httprequest.path or '/').rstrip('/') or '/'
                default_dbs = config['db_name']
                if path in ('/', '/web') and default_dbs and len(default_dbs) == 1:
                    default_db = default_dbs[0]
                    query = dict(self.httprequest.args) if self.httprequest.args else {}
                    query['db'] = default_db
                    return self.redirect_query('/web/login', query=query, code=303)
                exc.response = Response(
                    _not_found_nodb_body(), status=exc.code, headers=[
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = tcrm.api.Environment(cr, self.session.uid, self.session.context)
            try:
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~tcrm.http.Response.load` method for the compatible
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        _logger.exception("Internal Server Error: %s", exc)
        # In dev/debug show traceback in response so user can see the error
        if config.get('dev_mode') or (config.get('log_level') and config['log_level'] in ('debug', 'debug_sql', 'debug_rpc')):
            ser = serialize_exception(exc)
            body = f'<h1>Internal Server Error</h1><pre>{html.escape(ser["debug"])}</pre><p><b>{html.escape(ser["message"])}</b></p>'
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "tcrm Session Expired"
...
class Application:
    ...
    def initialize(self):
        ...
        from tcrm.service.server import load_server_wide_modules  # noqa: PLC0415
    ...
    def session_store(self):
        path = tcrm.tools.config.session_dir
    ...
    def geoip_city_db(self):
        geoip_city_db = config['geoip_city_db']
        if not geoip_city_db or not os.path.exists(geoip_city_db):
            _logger.debug("Geoip City file not found at %s. IP Resolver disabled.", geoip_city_db)
            return _EmptyGeoIPReader()
        try:
            return geoip2.database.Reader(geoip_city_db)
        except (OSError, maxminddb.InvalidDatabaseError):
            _logger.debug(
                "Couldn't load Geoip City file at %s. IP Resolver disabled.",
                geoip_city_db, exc_info=True
            )
            raise
    ...
    def geoip_country_db(self):
        ...
        if not geoip_country_db or not os.path.exists(geoip_country_db):
            _logger.debug("Geoip Country file not found at %s. Fallbacks on Geoip City.", geoip_country_db)
            return _EmptyGeoIPReader()
        try:
            return geoip2.database.Reader(geoip_country_db)
    ...
    def __call__(self, environ, start_response):
        ...
        if tcrm.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
        ...
        with HTTPRequest(environ) as httprequest:
            ...
            try:
                ...
                # Redirect / and /web to login with default db when no db selected
                # (avoids "No database is selected" 404 even if nodb routes failed to load)
                if not request.db:
                    path = (httprequest.path or '/').rstrip('/') or '/'
                    default_dbs = config['db_name']
                    if path in ('/', '/web') and default_dbs and len(default_dbs) == 1:
                        query = dict(httprequest.args) if httprequest.args else {}
                        query['db'] = default_dbs[0]
                        response = request.redirect_query('/web/login', query=query, code=303)
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/tcrm/')
                            or httprequest.path in (
                                '/tcrm', '/web', '/web/login', '/test_http/ensure_db',
                            )):
