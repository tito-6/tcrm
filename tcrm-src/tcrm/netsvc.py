...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "tcrm.netsvc.log is deprecated starting tcrm 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('tcrm.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('tcrm.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='tcrm.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("tcrm_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'tcrm.http.rpc.request:INFO',
    'tcrm.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['tcrm:DEBUG', 'tcrm.sql_db:INFO', 'tcrm.http.rpc:DEBUG'],
    'debug_rpc': ['tcrm:DEBUG', 'tcrm.sql_db:INFO', 'tcrm.http.rpc.request:DEBUG'],
    'debug': ['tcrm:DEBUG', 'tcrm.sql_db:INFO'],
    'debug_sql': ['tcrm.sql_db:DEBUG'],
    'info': [],
    'runbot': ['tcrm:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['tcrm:WARNING', 'werkzeug:WARNING'],
    'error': ['tcrm:ERROR', 'werkzeug:ERROR'],
    'critical': ['tcrm:CRITICAL', 'werkzeug:CRITICAL'],
}
...
def showwarning_with_traceback(message, category, filename, lineno, file=None, line=None):
    ...
    for frame in traceback.extract_stack():
        if frame.name == '__call__' and frame.filename.endswith('/tcrm/http.py'):
