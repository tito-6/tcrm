...
from tcrm import api, sql_db
from tcrm.modules.registry import Registry
from tcrm.release import nt_service_name
from tcrm.tools import config, gc, osutil, OrderedSet, profiler
from tcrm.tools.cache import log_ormcache_stats
from tcrm.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    import tcrm  # for eventd
    if tcrm.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(CommonRequestHandler):
    def setup(self):
        ...
        me.name = 'tcrm.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The tcrm_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("tcrm_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        import tcrm.addons  # noqa: PLC0415
        for path in tcrm.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="tcrm.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from tcrm.addons.base.models.ir_cron import IrCron  # noqa: PLC0415
    ...
    def cron_spawn(self):
        ...
        for i in range(config['max_cron_threads']):
            t = threading.Thread(target=self.cron_thread, args=(i,), name=f"tcrm.service.cron.cron{i}")
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from tcrm.tests.result import _logger as logger  # noqa: PLC0415
...
class PreforkServer(CommonServer):
    ...
    def start(self):
        ...
        if config['http_enable']:
            ...
            if os.environ.get('tcrm_HTTP_SOCKET_FD'):
                # reload
                self.socket = socket.socket(fileno=int(os.environ.pop('tcrm_HTTP_SOCKET_FD')))
    ...
    def fork_and_reload(self):
        ...
        if pid != 0:
            ...
            os.environ['tcrm_HTTP_SOCKET_FD'] = str(http_socket_fileno)
            os.environ['tcrm_READY_SIGHUP_PID'] = str(pid)
    ...
    def run(self, preload, stop):
        ...
        if os.environ.get('tcrm_READY_SIGHUP_PID'):
            os.kill(int(os.environ.pop('tcrm_READY_SIGHUP_PID')), signal.SIGHUP)
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('tcrm: %s %s %s' % (self.__class__.__name__, self.pid, title))
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The tcrm_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("tcrm_HTTP_SOCKET_TIMEOUT")
...
def load_server_wide_modules():
    from tcrm.modules.module import load_openerp_module  # noqa: PLC0415
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        if os.environ.get('tcrm_PROFILE_PRELOAD'):
            interval = float(os.environ.get('tcrm_PROFILE_PRELOAD_INTERVAL', '0.1'))
            collectors = [profiler.PeriodicCollector(interval=interval)]
            if os.environ.get('tcrm_PROFILE_PRELOAD_SQL'):
        ...
        try:
            with preload_profiler:
                ...
                if config['test_enable']:
                    from tcrm.tests import loader  # noqa: PLC0415
                    t0 = time.time()
...
def start(preload=None, stop=False):
    """ Start the tcrm http server and cron processor.
    """
    global server

    load_server_wide_modules()
    import tcrm.http  # noqa: PLC0415

    if tcrm.evented:
        server = GeventServer(tcrm.http.root)
    elif config['workers']:
        if config['test_enable']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(tcrm.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which tcrm is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(tcrm.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not tcrm.evented:
        if inotify:
