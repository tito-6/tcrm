...
from tcrm.release import author as __author__  # noqa: F401
from tcrm.release import version as __version__  # noqa: F401
from tcrm.service import server
from tcrm.tools import config

from . import Command

# Also use the `tcrm` logger for the main script.
_logger = logging.getLogger('tcrm')
...
def report_configuration():
    ...
    import tcrm.release  # noqa: PLC0415
    _logger.info("tcrm version %s", tcrm.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', tcrm.addons.__path__)
    ...
    if sys.version_info[:2] > tcrm.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, tcrm.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import tcrm  # for evented  # noqa: PLC0415
    if not tcrm.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from tcrm.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the tcrm server (default command)"""
