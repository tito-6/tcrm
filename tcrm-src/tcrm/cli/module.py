...
from tcrm.modules.loading import force_demo
from tcrm.modules.module import get_module_path, initialize_sys_path
from tcrm.modules.registry import Registry
from tcrm.tools import OrderedSet, config, parse_version
...
class Module(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        install_parser.epilog = textwrap.dedent("""\
            Before installing modules, an tcrm database needs to be created and initialized
            on your PostgreSQL instance, using the `db init` command:

            $ tcrm-bin db init <db_name>

            To get help on its parameters, see:

            $ tcrm-bin db init --help
        """)
