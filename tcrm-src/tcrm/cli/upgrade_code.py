...
* addon: the tcrm addon in which the file is;
...
try:
    ...
    from tcrm.modules import initialize_sys_path
    from tcrm.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via tcrm-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing tcrm
    ...
    class Command:
        ...
        def parser(self):
            return argparse.ArgumentParser(
                ...
                description=__doc__.replace('/tcrm/upgrade_code', str(UPGRADE)),
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /tcrm/upgrade_code """
    name = 'upgrade_code'
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = tcrm.addons.__path__
        else:
