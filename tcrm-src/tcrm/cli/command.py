...
import tcrm.cli
from tcrm.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``tcrm.cli`` """
    for path in tcrm.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'tcrm.cli.{module.stem}')
    ...
    for path in tcrm.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as tcrm.cli and not tcrm.addons.{module}.cli
                # so it doesn't load tcrm.addons.{module}.__init__
                mapping[f'tcrm.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from tcrm.cli
    with contextlib.suppress(ImportError):
        __import__(f'tcrm.cli.{name}')
        return commands[name]

    # import from tcrm.addons.*.cli
