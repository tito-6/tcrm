...
from tcrm import api, fields, models, tools, SUPERUSER_ID, _
from tcrm.fields import Command, Date, Domain
from tcrm.addons.rating.models import rating_data
from tcrm.addons.html_editor.tools import handle_history_divergence
from tcrm.exceptions import UserError, ValidationError
from tcrm.tools import format_list, SQL, LazyTranslate, html_sanitize
from tcrm.addons.resource.models.utils import filter_domain_leaf
from tcrm.addons.project.controllers.project_sharing_chatter import ProjectSharingChatter
...
class ProjectTask(models.Model):
    ...
    # Need this field to check there is no email loops when Tcrm reply automatically
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Tcrm-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Tcrm-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Tcrm-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
    ...
    def action_redirect_to_project_task_form(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': f"/tcrm/{self.project_id.id}/action-project.act_project_project_2_project_task_all/{self.id}?menu_id={menu_id}",
            'target': 'new',
        }
