...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{tcrm_url}/xmlrpc/2/common')
    uid = common.authenticate(tcrm_db, tcrm_username, tcrm_password, {})
    models = xmlrpc.client.ServerProxy(f'{tcrm_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(tcrm_db, uid, tcrm_password, 'ir.attachment', 'search_read', [
