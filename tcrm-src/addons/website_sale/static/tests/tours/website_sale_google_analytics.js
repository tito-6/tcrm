...
function patchTracking() {
    const { Tracking } = tcrm.loader.modules.get('@website_sale/interactions/tracking');
...
if (tcrm.loader.modules.has('@website_sale/interactions/tracking')) {
    patchTracking();
} else {
    tcrm.loader.bus.addEventListener('module-started', (e) => {
