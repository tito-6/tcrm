...
from tcrm.http import root
from tcrm.tests import HttpCase, tagged
from tcrm.tools import urls

from tcrm.addons.payment import utils as payment_utils
from tcrm.addons.website_sale.controllers.delivery import Delivery as WebsiteSaleDeliveryController
from tcrm.addons.website_sale.controllers.main import WebsiteSale
from tcrm.addons.website_sale.controllers.cart import Cart
from tcrm.addons.website_sale.tests.common import MockRequest, WebsiteSaleCommon
...
class TestWebsiteSaleExpressCheckoutFlows(WebsiteSaleCommon, HttpCase):
    ...
    def test_express_checkout_registered_user(self):
        """ Test that when you use express checkout as a registered user and the address sent by the
            express checkout form exactly matches the one registered in tcrm, we do not create a new
    ...
    def test_express_checkout_registered_user_existing_address(self):
        """ Test that when you use the express checkout as a registered user and the address sent by
            ...
            tcrm, we do not create a new partner and reuse the existing one.
        """
    ...
    def test_express_checkout_public_user_shipping_address_change(self):
        ...
        root.session_store.save(session)
        with patch(
            'tcrm.addons.delivery.models.delivery_carrier.DeliveryCarrier.rate_shipment',
    ...
    def test_express_checkout_registered_user_with_shipping_option(self):
        """ Test that when you use the express checkout as a registered user and the shipping
            ...
            to this user in tcrm, we do not create a new partner and reuse the existing one.
        """
        self.sale_order.partner_id = self.user_demo.partner_id.id
        session = self.authenticate(self.user_demo.login, self.user_demo.login)
        session['sale_order_id'] = self.sale_order.id
    ...
    def test_express_checkout_registered_user_with_shipping_option_new_address(self):
        """ Test that when you use the express checkout as a registered user and the shipping
            address sent by the express checkout form doesn't exist in tcrm, we create a new
            partner.
        """
        self.sale_order.partner_id = self.user_demo.partner_id.id
        session = self.authenticate(self.user_demo.login, self.user_demo.login)
