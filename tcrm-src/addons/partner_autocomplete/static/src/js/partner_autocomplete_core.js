...
import { onWillStart } from "@tcrm/owl";
...
export function usePartnerAutocomplete() {
    const keepLasttcrm = new KeepLast();
    ...
    /**
     * Use Tcrm Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    async function getSuggestions(value, isVAT, queryCountryId) {
        ...
        const suggestions = await keepLasttcrm.add(prom);
