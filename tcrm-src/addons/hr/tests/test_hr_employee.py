...
from tcrm.tests import Form, users, new_test_user, HttpCase, tagged, TransactionCase
from tcrm.addons.hr.tests.common import TestHrCommon
...
class TestHrEmployee(TestHrCommon):
    ...
    def test_employee_timezone(self):
        ...
        with mute_logger('tcrm.sql_db'), self.assertRaises(NotNullViolation):
            employee.tz = False
        ...
        user_1, user_2, user_3 = self.env['res.users'].create([
            {
                'name': 'Test User',
                'login': 'test_user',
                'email': 'test_user@tcrm.com',
            },
            {
                'name': 'Test User 2',
                'login': 'test_user_2',
                'email': 'test_user_2@tcrm.com',
                'create_employee': True,
            },
            {
                'name': 'Test User 3',
                'login': 'test_user_3',
                'email': 'test_user_3@tcrm.com',
    ...
    def test_set_user_on_new_employee(self):
        ...
        employee_form.user_id = self.res_users_hr_officer
        with mute_logger('tcrm.sql_db'), self.assertRaises(UniqueViolation), self.assertRaises(ValidationError):
            employee_form.save()
    ...
    def test_search_hr_employee_no_access(self):
        ...
        with self.assertNoLogs('tcrm.domains'):
            self.assertEqual(
                employee.ids,
...
class TestHrEmployeeLinks(HttpCase):
    def test_shared_private_link_permissions(self):
        ...
        with mute_logger('tcrm.http'):  # ignore raised RedirectWarning
            self.start_tour(
                f"/tcrm/employees/{employee_sonic.id}",
