...
import tcrm.http
from tcrm.tests import HOST, HttpCase
from tcrm.tools import DotDict, config, frozendict
from tcrm.tools.urls import urljoin as url_join
...
def MockRequest(
    ...
    request = Mock(
        ...
        type='http',
        future_response=tcrm.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            tcrm.http.get_default_session(),
            context={'lang': ''},
            force_website_id=website and website.id,
        ),
        geoip=tcrm.http.GeoIP('127.0.0.1'),
    ...
    if country_code or city_name:
        try:
            request.geoip._city_record = tcrm.http.geoip2.models.City(['en'], country=(country_code and {'iso_code': country_code}) or {}, city=(city_name and {'names': {'en': city_name}}) or {})
        except TypeError:
            request.geoip._city_record = tcrm.http.geoip2.models.City({'country': (country_code and {'iso_code': country_code}) or {}, 'city': (city_name and {'names': {'en': city_name}}) or {}})
    ...
    #
    #   router = tcrm.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        ...
        s.callback(tcrm.http._request_stack.pop)
        s.enter_context(patch('tcrm.http.root.get_db_router', router))
