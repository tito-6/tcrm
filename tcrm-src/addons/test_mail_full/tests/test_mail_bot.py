...
from tcrm.addons.mail.tests.common import MailCommon
from tcrm.addons.test_mail.tests.common import TestRecipients
from tcrm.tests import tagged
from tcrm.tools import mute_logger


@tagged("tcrmbot")
class Testtcrmbot(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.tcrmbot = cls.env.ref("base.partner_root")
        ...
        cls.tcrmbot_ping_body = f'<a href="http://tcrm.com/tcrm/res.partner/{cls.tcrmbot.id}" class="o_mail_redirect" data-oe-id="{cls.tcrmbot.id}" data-oe-model="res.partner" target="_blank">@tcrmBot</a>'
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('tcrm.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_tcrmbot()
        tcrmbot = self.env.ref("base.partner_root")
        tcrmbot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', tcrmbot.id)])
        self.assertEqual(len(tcrmbot_in_fetch_listeners), 1, 'tcrmbot should appear only once in channel_fetch_listeners')

    @mute_logger('tcrm.addons.mail.models.mail_mail')
    def test_tcrmbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.tcrmbot_ping_body, 'partner_ids': [self.tcrmbot.id, self.user_admin.partner_id.id]})
        ...
        # tcrmbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.tcrmbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last tcrmbot message instead
            sender=self.tcrmbot,
            answer=("@tcrmBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.tcrmbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.tcrmbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail')
    def test_tcrmbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @tcrmbot right now", 'partner_ids': []})
        self.assertNextMessage(
