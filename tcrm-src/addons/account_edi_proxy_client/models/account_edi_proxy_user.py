...
from .account_edi_proxy_auth import tcrmEdiProxyAuth
    ...
    def _make_request(self, url, params=False, *, auth_type: Literal['hmac', 'asymmetric'] = 'hmac'):
        ...
        try:
            response = requests.post(
                ...
                auth=tcrmEdiProxyAuth(user=self, auth_type=auth_type)).json()
        ...
        if proxy_error:
            ...
            if error_code == 'invalid_signature':
                raise AccountEdiProxyError(
                    error_code,
                    _("Failed to connect to Tcrm Access Point server. This might be due to another connection to Tcrm Access Point "
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # Tcrm side. We are for now forced to check the error message.
