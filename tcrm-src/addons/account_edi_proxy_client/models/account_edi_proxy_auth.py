...
class tcrmEdiProxyAuth(requests.auth.AuthBase):
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'tcrm-edi-client-id': self.id_client,
            'tcrm-edi-timestamp': timestamp,
        })
        message = self.__get_payload(request, timestamp)

        if self.auth_type == 'asymmetric' and self.private_key:
            request.headers.update({
                'tcrm-edi-signature': self.__sign_with_private_key(message),
                'tcrm-edi-signature-type': 'asymmetric'
            })
        elif self.refresh_token:
            request.headers.update({
                'tcrm-edi-signature': self.__sign_request_with_token(message),
                'tcrm-edi-signature-type': 'hmac'
            })
