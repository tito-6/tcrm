...
export class WebsiteBuilderClientAction extends Component {
    ...
    /**
     * This replaces the browser url (/tcrm/website...) with
    ...
    replaceBrowserUrl() {
        ...
        if (
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, "/tcrm");
            return;
        }
    ...
    reloadWebClient() {
        ...
        redirect(
            `/tcrm/action-website.website_preview?website_id=${encodeURIComponent(
                websiteId
    ...
    onKeydownRefresh(ev) {
        ...
        redirect(
            `/tcrm/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`
        );
    }
...
registry
    ...
    .add("html_builder.website_builder_action", ({ host, pathname }) => {
        const backendRoutes = ["/web", "/web/session/logout", "/tcrm"];
        return (
            host !== window.location.host ||
            (pathname &&
                (backendRoutes.includes(pathname) ||
                    pathname.startsWith("/@/") ||
                    pathname.startsWith("/tcrm/") ||
