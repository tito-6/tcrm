...
patch(Colibri.prototype, {
    protectSyncAfterAsync(interaction, name, fn) {
        ...
        return (...args) => {
            ...
            wysiwyg?.tcrmEditor.observerUnactive(fullName);
            const result = fn(...args);
            wysiwyg?.tcrmEditor.observerActive(fullName);
            return result;
        };
    },
    addListener(target, event, fn, options) {
        ...
        const wysiwyg = window.$?.("#wrapwrap").data("wysiwyg");
        let stealthFn = fn;
        if (wysiwyg?.tcrmEditor && !fn.isHandler && stealth) {
            const name = `${this.interaction.constructor.name}/${event}`;
            stealthFn = (...args) => {
                wysiwyg.tcrmEditor.observerUnactive(name);
                const result = fn(...args);
                wysiwyg.tcrmEditor.observerActive(name);
                return result;
            };
        }
        return super.addListener(target, event, stealthFn, options);
