    ...
    isFrontendUrl(url) {
        ...
        return (
            (browser.location.hostname === parsedUrl.hostname ||
                // Also check if the tcrm-hosted domain is the current domain of the url
                new RegExp(`^https?://${session.db}\\.tcrm\\.com(/.*)?$`).test(parsedUrl.origin)) &&
            !parsedUrl.pathname.startsWith("/tcrm") &&
