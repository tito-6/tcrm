...
class TestWebsiteAssets(tcrm.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/tcrm')
        check_asset()
        self.url_open(domain_2 + '/tcrm')
        check_asset()
        self.url_open(domain_1 + '/tcrm')
        check_asset()
        self.url_open(domain_2 + '/tcrm')
...
class TestWebAssets(tcrm.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with tcrm.tools.mute_logger('tcrm.addons.web.controllers.binary'):
            self.assertEqual(
