...
from tcrm.addons.base.tests.common import HttpCaseWithUserPortal
from tcrm.addons.http_routing.tests.common import MockRequest
from tcrm.addons.website.controllers.form import WebsiteForm
...
class TestWebsiteFormEditor(HttpCaseWithUserPortal):
    ...
    def test_website_form_contact_us_edition_with_email(self):
        self.start_tour('/tcrm', 'website_form_contactus_edition_with_email', login="admin")
    ...
    def test_website_form_contact_us_edition_no_email(self):
        ...
        self.start_tour('/tcrm', 'website_form_contactus_edition_no_email', login="admin")
        self.start_tour('/contactus', 'website_form_contactus_submit', login="portal")
        mail = self.env['mail.mail'].search([], order='id desc', limit=1)
        self.assertEqual(
            mail.email_to,
    ...
    def test_website_form_html_escaping(self):
        ...
        with MockRequest(self.env, website=website):
            WebsiteFormController.insert_record(
                ...
                {'email_from': 'tcrmbot@example.com', 'subject': 'John <b>Smith</b>', 'email_to': 'company@company.company'},
                "John <b>Smith</b>",
            )
            mail = self.env['mail.mail'].search([], order='id desc', limit=1)
