...
from tcrm import http, models, fields, tools, _
from tcrm.exceptions import AccessError, UserError
from tcrm.fields import Domain
from tcrm.http import request, SessionExpiredException
from tcrm.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from tcrm.tools.translate import LazyTranslate
from tcrm.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from tcrm.addons.portal.controllers.portal import pager as portal_pager
from tcrm.addons.portal.controllers.web import Home
from tcrm.addons.web.controllers.binary import Binary
from tcrm.addons.web.controllers.session import Session
from tcrm.addons.website.tools import get_base_domain
from tcrm.tools.json import scriptsafe as json
from tcrm.tools.translate import TRANSLATED_ELEMENTS
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/tcrm?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': tcrm.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/tcrm/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/tcrm/ir.ui.view/{page.get('view_id')}")
