...
test("TCRM.ACCOUNT.GROUP type", async function () {
    const { store: composer } = await makeStore(CellComposerStore);

    composer.startEdition("=TCRM.ACCOUNT.GROUP(");
    ...
    await animationFrame();
    expect(composer.currentContent).toBe('=TCRM.ACCOUNT.GROUP("asset_receivable"');
