...
featurePluginRegistry.add("tcrmAccountingAggregates", AccountingPlugin);
...
cellMenuRegistry.add("move_lines_see_records", {
    ...
    async execute(env, newWindow) {
        ...
        if ( func.functionName === "TCRM.PARTNER.BALANCE" ) {
            [partner_ids, codes, date_range, offset, companyId, includeUnposted] = parsed_args;
        } else if ( func.functionName === "TCRM.BALANCE.TAG" ) {
        ...
        if ( date_range?.value && !isEvaluationError(date_range.value) ) {
            ...
            if ( ["TCRM.PARTNER.BALANCE", "TCRM.RESIDUAL", "TCRM.BALANCE.TAG"].includes(func.functionName) ) {
        ...
        if ( func.functionName === "TCRM.BALANCE.TAG" ) {
        ...
        } else if ( func.functionName === "TCRM.PARTNER.BALANCE" ) {
