...
from tcrm import http, release, service
from tcrm.addons.iot_drivers.tools.system import (
    IOT_CHAR,
    IOT_RPI_CHAR,
    IOT_WINDOWS_CHAR,
    IS_RPI,
...
from tcrm.tools.func import reset_cached_properties
...
class IoTRestart(Thread):
    """
    Thread to restart tcrm server in IoT Box when we must return a answer before
    """
...
def require_db(function):
    ...
    def wrapper(*args, **kwargs):
        fname = f"<function {function.__module__}.{function.__qualname__}>"
        server_url = get_tcrm_server_url()
        iot_box_ip = get_ip()
...
def check_image():
    ...
    """
    try:
        response = requests.get('https://nightly.tcrm.com/master/iotbox/SHA1SUMS.txt', timeout=5)
...
def save_conf_server(url, token, db_uuid, enterprise_code, db_name=None):
    """
    Save server configurations in tcrm.conf
    ...
    get_tcrm_server_url.cache_clear()
...
@cache
def get_tcrm_server_url():
    """Get the URL of the linked Tcrm database.

    :return: The URL of the linked Tcrm database.
    :rtype: str or None
    """
...
def get_commit_hash():
    return subprocess.run(
        ['git', '--work-tree=/home/pi/tcrm/', '--git-dir=/home/pi/tcrm/.git', 'rev-parse', '--short', 'HEAD'],
...
def get_version(detailed_version=False):
    if IS_RPI:
        image_version = read_file_first_line('/var/tcrm/iotbox_version')
    elif IS_WINDOWS:
...
def delete_iot_handlers():
    ...
    try:
        ...
        filenames = [
            f"tcrm/addons/iot_drivers/iot_handlers/{file.relative_to(iot_handlers)}"
...
def download_iot_handlers(auto=True, server_url=None):
    """Get the drivers from the configured Tcrm server.
    ...
    :param server_url: The URL of the connected Tcrm database (provided by decorator).
    """
    ...
    delete_iot_handlers()
    path = path_file('tcrm', 'addons', 'iot_drivers', 'iot_handlers')
    zip_file.extractall(path)


def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "tcrm.addons.iot_drivers.iot_handlers.{handler_kind}.{handler_name}".\
        format(handler_kind=handler_kind, handler_name=handler_file_name.removesuffix('.py'))


def load_iot_handlers():
    """
    This method loads local files: 'tcrm/addons/iot_drivers/iot_handlers/drivers' and
    'tcrm/addons/iot_drivers/iot_handlers/interfaces'
...
def tcrm_restart(delay=0):
    """
    Restart Tcrm service
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows Tcrm
    server folder
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Tcrm\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Tcrm\\downloaded_file.zip', 'C:\\Program Files\\Tcrm\\new_folder'))
...
def update_conf(values, section='iot.box'):
    """Update tcrm.conf with the given key and value.
    ...
    """
    _logger.debug("Updating tcrm.conf with values: %s", values)
    conf = get_conf()

    if not conf.has_section(section):
        _logger.debug("Creating new section '%s' in tcrm.conf", section)
        conf.add_section(section)

    for key, value in values.items():
        conf.set(section, key, value) if value else conf.remove_option(section, key)

        with open(path_file("tcrm.conf"), "w", encoding='utf-8') as f:
            conf.write(f)


def get_conf(key=None, section='iot.box'):
    """Get the value of the given key from tcrm.conf, or the full config if no key is provided.
...
tcrm_start_time = time.monotonic()
system_start_time = tcrm_start_time - _get_system_uptime()
...
def toggle_remote_connection(token=""):
    ...
    if p.returncode == 0:
        subprocess.run(
            ['sudo', 'systemctl', 'restart' if token else "stop", 'tcrm-ngrok.service'],
