...
from tcrm.addons.iot_drivers.tools import helpers
from tcrm.addons.iot_drivers.tools.system import IS_TEST
from tcrm.netsvc import ColoredFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in tcrm.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, tcrm_server_url, active):
        """
        :param tcrm_server_url: Tcrm Server URL
        """
        super().__init__()
        self._tcrm_server_url = tcrm_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._tcrm_server_url:
    ...
    def _periodic_flush(self):
        tcrm_session = requests.Session()
        while self._tcrm_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(tcrm_session)

    def _flush_logs(self, tcrm_session):
        ...
        if not self._tcrm_server_url or queue_size == 0:
            return
        try:
            tcrm_session.post(
                self._tcrm_server_url + '/iot/log',
                data=empty_queue(),
                headers={'X-Tcrm-Database': self._db_name},
...
def get_tcrm_config_log_to_server_option():
...
def check_and_update_tcrm_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_tcrm_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_tcrm_server_url(), get_tcrm_config_log_to_server_option())
if not IS_TEST:
    ...
    # Set it in the 'root' logger, on which every logger (including tcrm) is a child
