...
from tcrm.addons.iot_drivers.driver import Driver
from tcrm.addons.iot_drivers.event_manager import event_manager
from tcrm.addons.iot_drivers.main import iot_devices
from tcrm.addons.iot_drivers.tools import helpers, route
...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = helpers.path_file('tcrm-keyboard-layouts.conf')
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('tcrm-keyboard-layouts.conf', json.dumps(data))
    ...
    def _is_scanner(self):
        ...
        file_path = helpers.path_file('tcrm-keyboard-is-scanner.conf')
        if file_path.exists():
            data = json.loads(file_path.read_text())
    ...
    def _keyboard_input(self, scancode):
        ...
        pressed key represented by its scancode to the connected Tcrm instance.
    ...
    def _save_is_scanner(self, data):
        ...
        helpers.write_file('tcrm-keyboard-is-scanner.conf', json.dumps(data))
