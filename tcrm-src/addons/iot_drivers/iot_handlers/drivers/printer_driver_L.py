...
from tcrm.addons.iot_drivers.controllers.proxy import proxy_drivers
from tcrm.addons.iot_drivers.iot_handlers.drivers.printer_driver_base import PrinterDriverBase
from tcrm.addons.iot_drivers.iot_handlers.interfaces.printer_interface_L import conn, cups_lock
from tcrm.addons.iot_drivers.main import iot_devices
from tcrm.addons.iot_drivers.tools import helpers, wifi, route
...
class PrinterDriver(PrinterDriverBase):
    ...
    def print_raw(self, data):
        ...
        try:
            with cups_lock:
                job_id = conn.createJob(self.device_identifier, 'Tcrm print job', {'document-format': CUPS_FORMAT_AUTO})
                conn.startDocument(self.device_identifier, job_id, 'Tcrm print job', CUPS_FORMAT_AUTO, 1)
    ...
    def print_status_zpl(self):
        ...
        title = "IoT Box Connected" if helpers.get_tcrm_server_url() else "IoT Box Status"
    ...
    def _printer_status_content(self):
        ...
        wan_quality = helpers.check_network("www.tcrm.com")
        ...
        title = b'IoT Box Connected' if helpers.get_tcrm_server_url() else b'IoT Box Status'
