...
export class TimeDialog extends Component {
    ...
    setup() {
        this.store = useStore();
        this.state = useState({
            tcrmUptimeSeconds: this.store.base.tcrm_uptime_seconds,
            systemUptimeSeconds: this.store.base.system_uptime_seconds,
        });
        setInterval(() => {
            this.state.tcrmUptimeSeconds += 1;
    ...
    startDateFromSeconds(uptimeInSeconds) {
        ...
        const tcrmUptimeMs = uptimeInSeconds * 1000;
        return new Date(currentTimeMs - tcrmUptimeMs).toUTCString();
    }
        ...
        <BootstrapDialog identifier="'time-dialog'" btnName="'View Uptime'">
            ...
            <t t-set-slot="body">
                <div class="d-flex flex-column gap-4">
                  <div>
                    <h5>Tcrm Service</h5>
                    <div>Running for <b t-out="secondsToHumanReadable(state.tcrmUptimeSeconds)"/></div>
                    <div class="text-secondary">Started at <b t-out="startDateFromSeconds(state.tcrmUptimeSeconds)"/></div>
                  </div>
