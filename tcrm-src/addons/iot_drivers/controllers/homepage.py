...
from tcrm.addons.iot_drivers.tools import certificate, helpers, route, upgrade, wifi
from tcrm.addons.iot_drivers.tools.system import IOT_SYSTEM, IS_RPI
from tcrm.addons.iot_drivers.main import iot_devices, unsupported_devices
from tcrm.addons.iot_drivers.connection_manager import connection_manager
from tcrm.tools.misc import file_path
from tcrm.addons.iot_drivers.server_logger import (
    check_and_update_tcrm_config_log_to_server_option,
    get_tcrm_config_log_to_server_option,
...
class IotBoxOwlHomePage(http.Controller):
    ...
    # ---------------------------------------------------------- #
    @route.iot_route('/iot_drivers/restart_tcrm_service', type='http', cors='*')
    def tcrm_service_restart(self):
        helpers.tcrm_restart(0)
        return json.dumps({
            'status': 'success',
            'message': 'Tcrm service restarted',
        })

    @route.iot_route('/iot_drivers/iot_logs', type='http', cors='*')
    def get_iot_logs(self):
        logs_path = "/var/log/tcrm/tcrm-server.log" if IS_RPI else Path().absolute().parent.joinpath('tcrm.log')
    ...
    def get_homepage_data(self):
        ...
        tcrm_server_url = helpers.get_tcrm_server_url() or ''
        tcrm_uptime_seconds = time.monotonic() - helpers.tcrm_start_time
        ...
        return json.dumps({
            ...
            'server_status': tcrm_server_url,
            'pairing_code': connection_manager.pairing_code,
            'new_database_url': connection_manager.new_database_url,
            'pairing_code_expired': connection_manager.pairing_code_expired and not tcrm_server_url,
            'six_terminal': six_terminal,
            'is_access_point_up': IS_RPI and wifi.is_access_point(),
            'network_interfaces': network_interfaces,
            'version': helpers.get_version(),
            'system': IOT_SYSTEM,
            'tcrm_uptime_seconds': tcrm_uptime_seconds,
    ...
    def get_version_info(self):
        ...
        return json.dumps({
            ...
            'tcrmIsUpToDate': current_commit == last_available_commit or not bool(helpers.get_tcrm_server_url()),
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "Tcrm's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_tcrm_server_url(),
            'is_log_to_server_activated': get_tcrm_config_log_to_server_option(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'tcrm_current_log_level': self._get_logger_effective_level_str(logging.getLogger('tcrm')),
    ...
    def connect_to_tcrm_server(self, token):
        if token:
            try:
    ...
    def update_log_level(self, name, value):
        ...
        if name == 'log-to-server':
            check_and_update_tcrm_config_log_to_server_option(value)

        name = name[len(IOT_LOGGING_PREFIX):]
        if name == 'root':
            self._update_logger_level('', value, AVAILABLE_LOG_LEVELS)
        elif name == 'tcrm':
            self._update_logger_level('tcrm', value, AVAILABLE_LOG_LEVELS)
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) Tcrm's configuration and logger to the given logger_name to the given level.
        ...
        tcrm_TOOL_CONFIG_HANDLER_NAME = 'log_handler'
        LOG_HANDLERS = (helpers.get_conf(tcrm_TOOL_CONFIG_HANDLER_NAME, section='options') or []).split(',')
        ...
        if not IS_NEW_LEVEL_PARENT:
            ...
            _logger.debug('Adding to tcrm config log_handler: %s', new_entry)
        conf[tcrm_TOOL_CONFIG_HANDLER_NAME] = ','.join(log_handlers_without_logger)
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Tcrm Iot logger given an IoT handler name
        ...
        """
        tcrm_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return tcrm_addon_handler_path in logging.Logger.manager.loggerDict and \
               logging.getLogger(tcrm_addon_handler_path)
