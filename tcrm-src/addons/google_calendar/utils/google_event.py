...
class GoogleEvent(abc.Set):
    ...
    Inspired by Tcrm recordset, one instance can be a single Google event or a
    ...
    def tcrm_id(self, env):
        self.tcrm_ids(env)  # load ids
        return self._tcrm_id

    def _meta_tcrm_id(self, dbname):
        """Returns the Tcrm id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_tcrm_id' % dbname)
        if o_id:
            return int(o_id)

    def tcrm_ids(self, env):
        ids = tuple(e._tcrm_id for e in self if e._tcrm_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_tcrm_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_tcrm_ids_from_metadata(env, model)

        return tuple(e._tcrm_id for e in self)

    def _load_tcrm_ids_from_metadata(self, env, model):
        unsure_tcrm_ids = tuple(e._meta_tcrm_id(env.cr.dbname) for e in self)
        tcrm_events = model.browse(_id for _id in unsure_tcrm_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Tcrm id.
        # Therefore, we only consider Tcrm records without google id when trying
        # to match events.
        o_ids = tcrm_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            tcrm_id = e._meta_tcrm_id(env.cr.dbname)
            if tcrm_id in o_ids:
                e._events[e.id]['_tcrm_id'] = tcrm_id

    def _load_tcrm_ids_from_db(self, env, model):
        tcrm_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in tcrm_events}  # {google_id: tcrm_id}
        existing_google_ids = tcrm_events.mapped('google_id')
        for e in self:
            tcrm_id = mapping.get(e.id)
            if tcrm_id:
                e._events[e.id]['_tcrm_id'] = tcrm_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Tcrm.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Tcrm (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Tcrm) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Tcrm several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_tcrm_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.tcrm_ids(env)
        events.tcrm_ids(env)

        return self.filter(lambda e: e._tcrm_id)
    ...
    def get_tcrm_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.tcrm_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.tcrm_id(self.env)).base_event_id
