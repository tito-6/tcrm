...
from tcrm.tools import email_normalize
from tcrm.sql_db import BaseCursor

from tcrm.addons.google_calendar.utils.google_event import GoogleEvent
from tcrm.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from tcrm.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Tcrm database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Tcrm.
...
class GoogleCalendarSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Tcrm DB (_sync_google2tcrm).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_tcrm2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2tcrm(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in Tcrm. Creates new recurrences, updates
        existing ones.

        :param google_events: Google recurrences to synchronize in Tcrm
        :param write_dates: A dictionary mapping Tcrm record IDs to their write dates.
        :param default_reminders:
        :return: synchronized tcrm recurrences
        """
        ...
        tcrm_values = [
            dict(self._tcrm_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_tcrm = self.with_context(dont_notify=True, skip_contact_description=True)._create_from_google(new, tcrm_values)
        cancelled = existing.cancelled()
        cancelled_tcrm = self.browse(cancelled.tcrm_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in Tcrm.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_tcrm += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_tcrm.exists()._cancel()
        synced_records = new_tcrm + cancelled_tcrm
        pending = existing - cancelled
        pending_tcrm = self.browse(pending.tcrm_ids(self.env)).exists()
        for gevent in pending:
            tcrm_record = self.browse(gevent.tcrm_id(self.env))
            if tcrm_record not in pending_tcrm:
                # The record must have been deleted in the mean time; nothing left to sync
                continue
            # Last updated wins.
            # This could be dangerous if google server time and tcrm server time are different
            updated = parse(gevent.updated)
            # Use the record's write_date to apply Google updates only if they are newer than Tcrm's write_date.
            tcrm_record_write_date = write_dates.get(tcrm_record.id, tcrm_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not tcrm_record_write_date or updated >= pytz.utc.localize(tcrm_record_write_date):
                vals = dict(self._tcrm_values(gevent, default_reminders), need_sync=False)
                tcrm_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= tcrm_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from tcrm to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from tcrm to Google, False otherwise. """
        is_active_clause = Domain(self._active_name, '=', True) if self._active_name else Domain.TRUE
        domain = self._get_sync_domain()
        domain &= (Domain('google_id', '=', False) & is_active_clause) | Domain('need_sync', '=', True)
    ...
    @api.model
    def _tcrm_values(self, google_event: GoogleEvent, default_reminders=()):
    ...
    def _is_google_insertion_blocked(self, sender_user):
        ...
        This is a necessary step for ensuring data match between Tcrm and Google,
