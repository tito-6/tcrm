import { Component, markup, whenReady, validate } from "@tcrm/owl";
    ...
    start: async (env, { orm, effect, overlay }) => {
        ...
        async function resumeTour() {
            ...
            if (tourConfig.mode === "auto") {
                if (!tcrm.loader.modules.get("@web_tour/js/tour_automatic/tour_automatic")) {
                    await loadBundle("web_tour.automatic", { css: false });
                }
                const { TourAutomatic } = tcrm.loader.modules.get(
                    "@web_tour/js/tour_automatic/tour_automatic"
                );
                new TourAutomatic(tour).start();
            } else {
                await loadBundle("web_tour.interactive");
                const { TourPointer } = tcrm.loader.modules.get(
                ...
                const { TourInteractive } = tcrm.loader.modules.get(
        ...
        async function tourRecorder() {
            ...
            const { TourRecorder } = tcrm.loader.modules.get(
        ...
        tcrm.startTour = startTour;
        tcrm.isTourReady = (tourName) => getTourFromRegistry(tourName).wait_for.then(() => true);
