...
import { queryAll, queryFirst, queryOne } from "@tcrm/hoot-dom";
import { Component, useState, useExternalListener } from "@tcrm/owl";
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hastcrmClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hastcrmClass;
        ...
        const tcrmClass = [...currentElem.classList].find((c) => c.match(tcrm_CLASS_REGEX));
        if (tcrmClass) {
            currentPredicate = `.${tcrmClass}`;
            hastcrmClass = true;
        }
