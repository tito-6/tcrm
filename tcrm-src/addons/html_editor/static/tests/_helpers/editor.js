...
import { destroy, expect, getFixture } from "@tcrm/hoot";
import { queryOne } from "@tcrm/hoot-dom";
import { Component, markup, onWillDestroy, xml } from "@tcrm/owl";
...
export async function setupWysiwyg(props = {}) {
    ...
    const el = /** @type {HTMLElement} **/ (
        queryOne(`${props.iframe ? ":iframe " : ""}.tcrm-editor-editable`)
    );
    if (content) {
...
export function insertTestHtml(innerHtml) {
    ...
    container.classList.add("tcrm-editor-editable");
