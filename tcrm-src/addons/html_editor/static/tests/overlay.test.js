import { beforeEach, expect, test, describe, getFixture } from "@tcrm/hoot";
import { setSelection } from "./_helpers/selection";
import { click, hover, queryOne, waitFor, waitForNone } from "@tcrm/hoot-dom";
import {
    contains,
    defineModels,
    fields,
...
import { Component, onMounted, onWillUnmount, xml } from "@tcrm/owl";
...
test("Toolbar should not overflow scroll container", async () => {
    ...
    const editable = queryOne(".tcrm-editor-editable");
...
test("Toolbar should be visible after scroll bar is added", async () => {
    ...
    const p = queryOne(".tcrm-editor-editable p");
...
test("Toolbar should not overflow scroll container at the bottom", async () => {
    ...
    const lastP = queryOne(".tcrm-editor-editable p:last-child");
...
describe("powerbox", () => {
    ...
    test("Powerbox should be visible in a editable with small height", async () => {
        ...
        setSelection({ anchorNode: queryOne(".tcrm-editor-editable p"), anchorOffset: 1 });
        insertText(editor, "/");
    ...
    test("Powerbox should be visible in a editable with small height (2)", async () => {
        ...
        const thirdP = queryOne(".tcrm-editor-editable p:nth-child(3)");
...
test("Table column control should always be displayed on top of the table", async () => {
    ...
    const scrollableElement = queryOne(".o_content");
    const table = queryOne(".tcrm-editor-editable table");
    await hover(".tcrm-editor-editable td");
