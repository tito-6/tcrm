...
export class ClipboardPlugin extends Plugin {
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePastetcrmEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePastetcrmEditorHtml(clipboardData) {
        const tcrmEditorHtml = clipboardData.getData("application/vnd.tcrm.tcrm-editor");
        const textContent = clipboardData.getData("text/plain");
        if (ONLY_LINK_REGEX.test(textContent)) {
            return false;
        }
        if (tcrmEditorHtml) {
            const fragment = parseHTML(this.document, tcrmEditorHtml);
    ...
    async onDrop(ev) {
        ...
        const tcrmEditorHtml = ev.dataTransfer.getData("application/vnd.tcrm.tcrm-editor");
        const fileTransferItems = !tcrmEditorHtml && getImageFiles(dataTransfer);
        const htmlTransferItem = [...dataTransfer.items].find((item) => item.type === "text/html");
        if (fileTransferItems.length || htmlTransferItem || tcrmEditorHtml) {
