...
from tcrm.exceptions import AccessDenied, AccessError, UserError
from tcrm.fields import Domain
from tcrm.http import request
from tcrm.modules.module import MANIFEST_NAMES, Manifest
from tcrm.release import major_version
from tcrm.tools import SQL, convert_file
from tcrm.tools import file_open, file_path, file_open_temporary_directory, ormcache
from tcrm.tools.misc import OrderedSet, topological_sort
from tcrm.tools.translate import JAVASCRIPT_TRANSLATION_COMMENT, CodeTranslations, TranslationImporter, get_base_langs

from tcrm.addons.base.models.ir_asset import is_wildcard_glob

_logger = logging.getLogger(__name__)

APPS_URL = "https://apps.tcrm.com"
...
class IrModuleModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            ...
            raise UserError(_("Studio customizations require the Tcrm Studio app."))
    ...
    def _get_missing_dependencies(self, zip_data):
        ...
        if unavailable_modules:
            ...
            description += _(
                ...
                "https://www.tcrm.com/pricing-plan for more information.\n"
                "If you need Website themes, it can be downloaded from https://github.com/tcrm/design-themes.\n"
            )
        else:
            description = _(
    ...
    def _extract_resource_attachment_translations(self, module, lang):
        ...
        for attachment in attachments.filtered('raw'):
            ...
            if attachment.url.endswith('js'):
                extract_method = 'tcrm.tools.babel:extract_javascript'
                extract_keywords = {'_t': None}
            else:
                extract_method = 'tcrm.tools.translate:babel_extract_qweb'
                extract_keywords = {}
            try:
                with io.BytesIO(attachment.raw) as fileobj:
