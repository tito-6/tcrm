...
from tcrm.tools import exception_to_unicode
...
class tcrmIMAP4_SSL(tcrmIMAP4, IMAP4_SSL):
    pass
...
class tcrmPOP3_SSL(tcrmPOP3, POP3_SSL):
    pass
...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/tcrm-mailgate.py')
...
tcrm-mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
Example configuration for the postfix mta running locally:
/etc/postfix/virtual_aliases: @youdomain tcrm_mailgate@localhost
/etc/aliases:
tcrm_mailgate: "|/path/to/tcrm-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
    ...
    def _connect__(self, allow_archived=False):  # noqa: PLW3201
        ...
        if connection_type == 'imap':
            ...
            connection = tcrmIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else tcrmIMAP4(server, port, timeout=MAIL_TIMEOUT)
            self._imap_login__(connection)
        elif connection_type == 'pop':
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = tcrmPOP3_SSL(server, port, timeout=MAIL_TIMEOUT) if is_ssl else tcrmPOP3(server, port, timeout=MAIL_TIMEOUT)
