...
from tcrm import _, api, Command, fields, models, modules, tools
from tcrm.exceptions import UserError
from tcrm.http import request
from tcrm.tools import email_normalize, str2bool
...
class ResUsers(models.Model):
    ...
    notification_type = fields.Selection([
        ...
        help="Policy on how to handle Chatter notifications:\n"
             ...
             "- In Tcrm: notifications appear in your Tcrm Inbox")
    ...
    _notification_type = models.Constraint(
        ...
        'Only internal user can receive notifications in Tcrm',
    )
    ...
    def _init_store_data(self, store: Store):
        ...
        # sudo: res.partner - exposing tcrmBot data is considered acceptable
        tcrmbot = self.env.ref("base.partner_root").sudo()
        if not self.env.user._is_public():
            tcrmbot = tcrmbot.with_prefetch((tcrmbot + self.env.user.partner_id).ids)
