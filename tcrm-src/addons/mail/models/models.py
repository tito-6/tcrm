...
from tcrm import api, exceptions, models, tools, _
from tcrm.addons.mail.tools.alias_error import AliasError
from tcrm.tools import parse_contact_from_email
from tcrm.tools.mail import email_normalize, email_split_and_format
from tcrm.tools.sql import column_exists
...
class Base(models.AbstractModel):
    ...
    def _message_get_default_recipients(self, with_cc=False, all_tos=False):
        ...
        # ban emails: never propose tcrmbot nor aliases
        all_emails = []
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if not self:
            return headers
        self.ensure_one()
        headers['X-Tcrm-Objects'] = f"{self._name}-{self.id}"
