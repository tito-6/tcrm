...
import { describe, test } from "@tcrm/hoot";
import {
    asyncStep,
    Command,
    onRpc,
...
test("Auto-open tcrmBot chat when opening discuss for the first time", async () => {
    // tcrmbot chat has onboarding for using Discuss app.
    // We assume pinned tcrmbot chat without any message seen means user just started using Discuss app.
    const pyEnv = await startServer();
    ...
    await start();
    await openDiscuss();
    await contains(".o-mail-DiscussContent-threadName", { value: "tcrmBot" });
});
