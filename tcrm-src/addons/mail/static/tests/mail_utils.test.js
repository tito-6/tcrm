...
test("add_link utility function", () => {
    const testInputs = {
        ...
        "https://www.transifex.com/tcrm/tcrm-11/translate/#fr/lunch?q=text%3A'La+Tartiflette'": true,
        "https://www.transifex.com/tcrm/tcrm-11/translate/#fr/$/119303430?q=text%3ATartiflette": true,
        "https://tenor.com/view/chỗgiặt-dog-smile-gif-13860250": true,
        "http://www.boîtenoire.be": true,
        "https://github.com/tcrm/enterprise/compare/16.0...tcrm-dev:enterprise:16.0-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/tcrm/enterprise/compare/16.0...16.0-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/tcrm/enterprise/compare/16.0...chỗgiặt-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/tcrm/enterprise/compare/chỗgiặt...chỗgiặt-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/tcrm/enterprise/compare/@...}-voip-fix_demo_data-tsm?expand=1": true,
        "https://x.com": true,
    };
...
test("addLink: utility function and special entities", () => {
    const testInputs = [
        ...
        [
            markup`https://tcrm.com/%5B%5D`,
            `<a target="_blank" rel="noreferrer noopener" href="https://tcrm.com/%5B%5D">https://tcrm.com/[]</a>`,
        ],
    ];
...
test("url", async () => {
    ...
    const messageBody = "https://tcrm.com?test=~^|`{}[]#";
...
test("url with comma at the end", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://tcrm.com, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await press("Enter");
    await contains(".o-mail-Message a:contains(https://tcrm.com)");
...
test("url with number in subdomain", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.tcrm.com/tcrm";
    await insertText(".o-mail-Composer-input", messageBody);
    await press("Enter");
    await contains(
        ".o-mail-Message a:contains(https://www.45017478-master-all.runbot134.tcrm.com/tcrm)"
    );
});
