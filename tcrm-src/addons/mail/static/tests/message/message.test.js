...
import { animationFrame, leave, pointerDown, press, queryFirst, waitFor } from "@tcrm/hoot-dom";
import { advanceTime, mockDate, mockTouch, mockUserAgent, tick } from "@tcrm/hoot-mock";
import {
    asyncStep,
...
test("edit a message with styling keeps the styling", async () => {
    ...
    await click(".o-mail-Message [title='Edit']");
    await contains(".o-mail-Composer-html.tcrm-editor-editable strong:contains('Hello world')");
});
...
test("edit a message styling", async () => {
    ...
    await click(".o-mail-Message [title='Edit']");
    await focus(".o-mail-Message .o-mail-Composer-html.tcrm-editor-editable");
    const editor = {
        document,
        editable: document.querySelector(
            ".o-mail-Message .o-mail-Composer-html.tcrm-editor-editable"
        ),
    };
...
test("Update the link previews when a message is edited", async () => {
    ...
    await openDiscuss(channelId);
    await click(".o-mail-Message [title='Edit']");
    await insertText(".o-mail-Message .o-mail-Composer-input", "http://tcrm.com", {
        replace: true,
    });
    await click(".o-mail-Message button", { text: "save" });
    await contains(".o-mail-Message-body", { text: "http://tcrm.com" });
...
test("should delete link preview along with message", async () => {
    ...
    const linkPreviewId = pyEnv["mail.link.preview"].create({
        ...
        og_type: "article",
        source_url: "https://www.tcrm.com",
    });
    const channelId = pyEnv["discuss.channel"].create({ name: "PreviewTest" });
    pyEnv["mail.message"].create({
        body: "<a href='https://www.tcrm.com'>https://www.tcrm.com</a>",
