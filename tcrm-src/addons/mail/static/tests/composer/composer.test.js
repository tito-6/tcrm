...
import { Deferred, animationFrame, tick } from "@tcrm/hoot-mock";
import {
    Command,
    getService,
    onRpc,
...
import { edit, press, queryFirst } from "@tcrm/hoot-dom";
...
test("composer text input placeholder should contain channel name when thread does not have specific correspondent", async () => {
    ...
    await openDiscuss(channelId);
    await contains(
        ".o-mail-Composer-html.tcrm-editor-editable .o-we-hint[o-we-hint-text='Message #General…']"
    );
});
...
test("composer input placeholder in channel thread", async () => {
    ...
    await openDiscuss(subchannelID);
    await contains(
        ".o-mail-Composer-html.tcrm-editor-editable .o-we-hint[o-we-hint-text='Message \"ThreadFromGeneral\"']"
    );
});
...
test("add an emoji", async () => {
    ...
    await click("button[title='Add Emojis']");
    await click(".o-Emoji", { text: "😤" });
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { text: "😤" });
});
...
test("Exiting emoji picker brings the focus back to the Composer textarea", async () => {
    ...
    composerService.setHtmlComposer();
    await click("button[title='Add Emojis']");
    await contains(".o-mail-Composer-html.tcrm-editor-editable:not(:focus)");
    triggerHotkey("Escape");
    await contains(".o-mail-Composer-html.tcrm-editor-editable:focus");
});
...
test("add an emoji after some text", async () => {
    ...
    const composerService = getService("mail.composer");
    composerService.setHtmlComposer();
    await focus(".o-mail-Composer-html.tcrm-editor-editable");
    const editor = {
        document,
        editable: document.querySelector(".o-mail-Composer-html.tcrm-editor-editable"),
    };
    await htmlInsertText(editor, "Blabla");
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { text: "Blabla" });
    await click("button[title='Add Emojis']");
    await click(".o-Emoji", { text: "🤑" });
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { text: "Blabla🤑" });
});
...
test("composer text input placeholder should contain correspondent name when thread has exactly one correspondent", async () => {
    ...
    composerService.setHtmlComposer();
    await contains(
        ".o-mail-Composer-html.tcrm-editor-editable .o-we-hint[o-we-hint-text='Message Marc Demo…']"
    );
});
...
test("composer: paste attachments", async () => {
    ...
    composerService.setHtmlComposer();
    await openDiscuss(channelId);
    await contains(".o-mail-Composer-html.tcrm-editor-editable");
    await contains(".o-mail-AttachmentList .o-mail-AttachmentContainer", { count: 0 });
    await pasteFiles(".o-mail-Composer-html.tcrm-editor-editable", [text]);
    await contains(
        ".o-mail-AttachmentList .o-mail-AttachmentContainer:not(.o-isUploading):contains(text.txt)"
    );
});
...
test("select a canned response suggestion", async () => {
    ...
    await htmlInsertText(editor, "::");
    await click(".o-mail-Composer-suggestion");
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { text: "Hello! How are you?" });
});
...
test("select a canned response suggestion with some text", async () => {
    ...
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { text: "bluhbluh" });
    await htmlInsertText(editor, "::");
    await click(".o-mail-Composer-suggestion");
    await contains(".o-mail-Composer-html.tcrm-editor-editable", {
...
test("html composer: basic rendering", async () => {
    ...
    await click("button", { text: "Send message" });
    await contains("textarea.o-mail-Composer-input");
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { count: 0 });
    await insertText(".o-mail-Composer-input", "Hello World!");
    composerService.setHtmlComposer();
    await contains("textarea.o-mail-Composer-input", { count: 0 });
    await contains(".o-mail-Composer-html.tcrm-editor-editable", { text: "Hello World!" });
    await focus(".o-mail-Composer-html.tcrm-editor-editable");
    const editor = {
        document,
        editable: document.querySelector(".o-mail-Composer-html.tcrm-editor-editable"),
