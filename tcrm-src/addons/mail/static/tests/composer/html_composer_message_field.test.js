...
test("media dialog: upload", async function () {
    ...
    const anchorNode = queryOne(".tcrm-editor-editable div.o-paragraph");
...
test("mention a partner", async () => {
    ...
    const anchorNode = queryOne(`.tcrm-editor-editable p`);
    setSelection({ anchorNode, anchorOffset: 0 });
    ...
    await press("enter");
    expect("[name='body'] .tcrm-editor-editable").toHaveInnerHTML(`
    <p>
        <a target="_blank" data-oe-protected="true" contenteditable="false" href="https://www.hoot.test/tcrm/res.partner/17" class="o_mail_redirect" data-oe-id="17" data-oe-model="res.partner">
            @Mitchell Admin
        </a>
    </p>`);
});
...
test("mention a channel", async () => {
    ...
    const anchorNode = queryOne(`[name='body'] .tcrm-editor-editable div.o-paragraph`);
    setSelection({ anchorNode, anchorOffset: 0 });
    await insertText(htmlEditor, "#");
    await animationFrame();
...
describe("Remove attachments", () => {
    ...
    test("should remove file from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".tcrm-editor-editable");
        const anchorNode = queryOne(".tcrm-editor-editable div.o-paragraph");
        setSelection({ anchorNode, anchorOffset: 0 });
        ...
        await waitFor(".tcrm-editor-editable .o_file_box:has(a:contains('file.txt'))");
        await click("[name='attachment_ids'] button:has(i.fa-times)");
        await waitForNone("[name='attachment_ids'] a:contains('file.txt')");
        await waitForNone(".tcrm-editor-editable .o_file_box:has(a:contains('file.txt'))");
    });
    ...
    test("should remove image from html editor if removed from attachment list", async () => {
        ...
        await waitFor(".tcrm-editor-editable img[data-attachment-id='1']");
        await click("[name='attachment_ids'] button:has(i.fa-times)");
        await waitForNone("[name='attachment_ids'] a:contains('test.jpg')");
        await waitForNone(".tcrm-editor-editable img[data-attachment-id='1']");
    });
});
