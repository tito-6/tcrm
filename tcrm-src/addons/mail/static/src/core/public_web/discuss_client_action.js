...
import { Component, onMounted, onWillStart, onWillUnmount, onWillUpdateProps } from "@tcrm/owl";
...
export class DiscussClientAction extends Component {
    ...
    async restoreDiscussThread(props) {
        ...
        if (!parsedActiveId) {
            ...
            const tcrmbotChat = this.store.tcrmbot?.searchChat();
            const selfMember = tcrmbotChat?.self_member_id;
            if (tcrmbotChat && selfMember?.is_pinned && !selfMember.seen_message_id) {
                tcrmbotChat.setAsDiscussThread(false);
            }
            return;
        }
