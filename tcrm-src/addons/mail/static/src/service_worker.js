...
let db;
const unread_store = new Store("tcrm-mail-unread-db", "tcrm-mail-unread-store");
 ...
 * @param {Object} param1
 * @param {string} [param1.action] tcrm client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    const discussURLRegexes = [new RegExp("/tcrm/discuss")];
    if (action) {
        discussURLRegexes.push(
            new RegExp(`/tcrm/\\d+/action-${action}`),
            new RegExp(`/tcrm/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/tcrm/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/tcrm/${modelPath}/${res_id}`));
        }
    }
});
