...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="tcrm.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="tcrm.exceptions.AccessError"), mute_logger("tcrm.http"):
            self._mock_translation_request({"message_id": self.message.id})
