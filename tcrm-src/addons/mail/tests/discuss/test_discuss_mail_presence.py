...
from tcrm.tests import tagged, new_test_user
from tcrm.addons.bus.tests.common import WebsocketCase
from tcrm.addons.mail.tests.common import MailCommon, freeze_all_time
from tcrm.addons.bus.models.bus import channel_with_db, json_dump
...
class TestMailPresence(WebsocketCase, MailCommon):
    def _receive_presence(self, requested_by, target, has_token=False):
        ...
        channel_parts = ["tcrm-presence", f"{target_channel._name}_{target_channel.id}"]
        if has_token:
