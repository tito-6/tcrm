...
import { highlightText, tcrmmark } from "@web/core/utils/html";
...
export class Many2XAutocomplete extends Component {
    ...
    onQuickCreateError(error, request) {
        if (
            ...
            error.exceptionName === "tcrm.exceptions.ValidationError"
        ) {
    ...
    buildRecordSuggestion(request, record) {
        ...
        return {
            ...
            label: label
                ? highlightText(request, tcrmmark(label), "text-primary fw-bold")
                : _t("Unnamed"),
