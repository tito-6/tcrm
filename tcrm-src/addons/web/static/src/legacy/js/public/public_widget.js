...
function tcrmEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
tcrmEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
tcrmEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Tcrm where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new tcrmEvent(this, name, info);
