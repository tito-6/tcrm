...
import { Component, useRef, useState, markup } from "@tcrm/owl";
...
export const tcrmExceptionTitleMap = new Map(
    Object.entries({
        "tcrm.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "tcrm.exceptions.AccessDenied": _t("Access Denied"),
        "tcrm.exceptions.MissingError": _t("Missing Record"),
        "tcrm.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "tcrm.addons.base.models.ir_actions.ServerActionWithWarningsError": _t("Invalid Operation"),
        "tcrm.exceptions.UserError": _t("Invalid Operation"),
        "tcrm.exceptions.ValidationError": _t("Validation Error"),
        "tcrm.exceptions.AccessError": _t("Access Error"),
        "tcrm.exceptions.Warning": _t("Warning"),
    })
);
...
ClientErrorDialog.title = _t("Tcrm Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("Tcrm Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && tcrmExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = tcrmExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("Tcrm Server Error");
                break;
            case "script":
                this.title = _t("Tcrm Client Error");
                break;
            case "network":
                this.title = _t("Tcrm Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && tcrmExceptionTitleMap.has(this.props.exceptionName)) {
            return tcrmExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("Tcrm Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    ...
    setup() {
        ...
        this.title = capitalize(subType) || _t("Tcrm Warning");
        this.message = message;
...
registry
    ...
    .add("tcrm.exceptions.AccessDenied", WarningDialog)
    .add("tcrm.exceptions.AccessError", WarningDialog)
    .add("tcrm.exceptions.MissingError", WarningDialog)
    .add("tcrm.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("tcrm.addons.base.models.ir_actions.ServerActionWithWarningsError", WarningDialog)
    .add("tcrm.exceptions.UserError", WarningDialog)
    .add("tcrm.exceptions.ValidationError", WarningDialog)
    .add("tcrm.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("tcrm.http.SessionExpiredException", SessionExpiredDialog)
