...
export function startUrl() {
    return isScopedApp() ? "scoped_app" : "tcrm";
}
...
function urlToState(urlObj) {
    ...
    if (["tcrm", "scoped_app"].includes(prefix)) {
        ...
        if (prefix === "scoped_app" && !isDisplayStandalone()) {
            // make sure /scoped_app are redirected to /tcrm when using the browser instead of the PWA
            const url = browser.location.origin + router.stateToUrl(state);
            urlObj.href = url;
        }
    }
 ...
 * external website). Unfortunately, Tcrm wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/tcrm") &&
            (["/web", "/tcrm"].includes(url.pathname) || url.pathname.startsWith("/tcrm/")) &&
            a.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/tcrm") && url.hash) {
