...
const cacheName = "tcrm-sw-cache";
const homepageURL = "/tcrm";
...
const extractSessionInfo = (htmlContent) => {
    const match = htmlContent.match(/tcrm\.__session_info__\s*=\s*({.*?});/s);
...
const readDataOnCache = async (url) => {
    ...
    // if you come from /tcrm to project the url is now /tcrm/project, but it doesn't exist in cache so use /tcrm instead
    if (!response) {
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/tcrm?share_target=trigger"));
    event.waitUntil(
        (async () => {
            // The page sends this message to tell the service worker it's ready to receive the file.
            await waitingMessage("tcrm_share_target");
            const client = await self.clients.get(event.resultingClientId || event.clientId);
            const data = await event.request.formData();
            client.postMessage({
                shared_files: data.getAll("externalMedia") || [],
                action: "tcrm_share_target_ack",
            });
        })()
    );
};
