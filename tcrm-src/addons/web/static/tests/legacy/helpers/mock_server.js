/** @tcrm-module alias=@web/../tests/helpers/mock_server default=false */
...
export function makeServerError({
    ...
    return makeErrorFromResponse({
        code: code || 0,
        message: message || "Tcrm Server Error",
        data: {
            name: errorName || `tcrm.exceptions.${type || "UserError"}`,
            debug: "traceback",
            arguments: args || [],
            context: context || {},
            subType,
    ...
    mockLoadAction({ action_id } = {}) {
        ...
        if (!action) {
            throw makeServerError({
                errorName: "tcrm.addons.web.controllers.action.MissingActionError",
