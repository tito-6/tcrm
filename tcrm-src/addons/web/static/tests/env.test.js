import { after, beforeEach, describe, expect, getFixture, test } from "@tcrm/hoot";
 ...
 * @param {(env: import("@web/env").tcrmEnv, dependencies: Record<string, any>) => any} factory
 */
...
test(`mountComponent creates an env and sets the application as root when no env is provided`, async () => {
    ...
    after(() => {
        delete tcrm.__WOWL_DEBUG__;
    });
    const { env } = app;
    expect(env.services).toEqual({ my_service: "a" });
    expect(tcrm.__WOWL_DEBUG__).toEqual({ root: app.root.component });
...
test(`mountComponent uses the env when provided and doesn't start the services`, async () => {
    ...
    expect(tcrm.__WOWL_DEBUG__).toBe(undefined);
    expect(getFixture()).toHaveText("Root");
});
