...
import { click, queryAllTexts, waitFor } from "@tcrm/hoot-dom";
...
test("A new form view can be reloaded after a failed one", async () => {
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/tcrm/action-3/1");
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/tcrm/action-3/2");
