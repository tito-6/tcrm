...
import { animationFrame, runAllTimers } from "@tcrm/hoot-mock";
...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /tcrm?test=42",
        "window_reload",
        "pushState /tcrm/action-2",
        "window_reload",
        "pushState /tcrm?menu_id=1",
        "window_reload",
        "pushState /tcrm/action-1?menu_id=2",
        "window_reload",
    ]);
});
