import { after, beforeEach, expect, getFixture, test } from "@tcrm/hoot";
import {
    click,
    edit,
    on,
...
import { animationFrame, Deferred, mockSendBeacon, mockTouch, runAllTimers } from "@tcrm/hoot-mock";
import {
    clickModalButton,
    clickSave,
    defineActions,
...
test("change setting on nav bar click in base settings on desktop", async () => {
            ...
            <form string="Settings" class="oe_form_configuration o_base_settings" js_class="base_settings">
                <app string="CRM" name="crm">
                    ...
                    <block title="Title of group Foo">
                        <setting help="this is foo" info="this is foo info" documentation="https://www.tcrm.com/documentation/1.0/applications/technical/web/settings/this_is_another_test.html">
                            <field name="foo"/>
                        </setting>
    ...
    expect(".o_setting_box a:eq(0)").toHaveAttribute(
        "href",
        "https://www.tcrm.com/documentation/1.0/applications/technical/web/settings/this_is_a_test.html"
    );
    expect(".o_setting_box a:eq(1)").toHaveAttribute(
        "href",
        "https://www.tcrm.com/documentation/1.0/applications/technical/web/settings/this_is_another_test.html"
    );
...
test("settings views does not write the id on the url", async () => {
    ...
    await getService("action").doAction(1);
    await runAllTimers();
    expect(browser.location.pathname).toBe("/tcrm/settings");
    expect(".o_field_boolean input").toHaveProperty("disabled", false);
    await click(".o_field_boolean input");
    await animationFrame();
...
test("Open settings from url, with setting id anchor", async () => {
    ...
    redirect("/tcrm/settings#setting_id");
