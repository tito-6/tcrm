...
function invalidImportsFrom(folder, allowedFolders) {
    ...
    const modulesToCheck = Array.from(tcrm.loader.modules.keys()).filter((module) =>
        module.startsWith(`@web/${folder}/`)
    );
    const invalidDeps = {};
    for (const module of modulesToCheck) {
        const invalid = tcrm.loader.factories.get(module).deps.filter((dep) => {
            // owl and @web/session are allowed everywhere
            if (dep === "@tcrm/owl" || dep === "@web/session") {
                return false;
            }
