...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.tcrm.com">tcrm<a>`)).toBe(
        `&lt;a href=&quot;https://www.tcrm.com&quot;&gt;tcrm&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.tcrm.com'>tcrm<a>`)).toBe(
        `&lt;a href=&#x27;https://www.tcrm.com&#x27;&gt;tcrm&lt;a&gt;`
    );
    expect(escape("<a href='https://www.tcrm.com'>Tcrm`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.tcrm.com&#x27;&gt;Tcrm&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.tcrm]")).toBe("\\[wowl\\.tcrm\\]");
    expect(escapeRegExp("^tcrm.define([.]*)$")).toBe("\\^tcrm\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@tcrm")).toBe(false);
    expect(isEmail("test@tcrm@tcrm.com")).toBe(false);
    expect(isEmail("te st@tcrm.com")).toBe(false);

    expect(isEmail("test@tcrm.com")).toBe(true);
});
