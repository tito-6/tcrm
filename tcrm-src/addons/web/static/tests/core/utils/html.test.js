import { htmlEscape, markup } from "@tcrm/owl";
...
test("tcrmmark", () => {
    expect(tcrmmark("").toString()).toBe("");
    expect(tcrmmark("**test**").toString()).toBe("<b>test</b>");
    expect(tcrmmark("**test** something else **test**").toString()).toBe(
        "<b>test</b> something else <b>test</b>"
    );
    expect(tcrmmark("--test--").toString()).toBe("<span class='text-muted'>test</span>");
    expect(tcrmmark("--test-- something else --test--").toString()).toBe(
        "<span class='text-muted'>test</span> something else <span class='text-muted'>test</span>"
    );
    expect(tcrmmark("`test`").toString()).toBe(
    ...
    expect(tcrmmark("test\ttest2").toString()).toBe(
        `test<span style="margin-left: 2em"></span>test2`
    );
    expect(tcrmmark("test\ntest2").toString()).toBe("test<br/>test2");
    expect(tcrmmark("<p>**test**</p>").toString()).toBe("&lt;p&gt;<b>test</b>&lt;/p&gt;");
    expect(tcrmmark(markup`<p>**test**</p>`).toString()).toBe("<p><b>test</b></p>");
});
