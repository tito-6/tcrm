...
import { queryAllAttributes, queryAllTexts, queryFirst } from "@tcrm/hoot-dom";
import {
    contains,
    defineModels,
    fieldInput,
...
test("UrlField in form view", async () => {
    ...
    await fieldInput("url").edit("https://www.tcrm.com");
    expect(`.o_field_widget input[type="text"]`).toHaveValue("https://www.tcrm.com");
});
...
test("onchange scenario", async () => {
    ...
    Product._records = [{ id: 1, url: "tcrm.com", url_source: "another.com" }];
    await mountView({
        type: "form",
        resModel: "product",
        resId: 1,
    ...
    expect(".o_field_widget[name=url]").toHaveText("tcrm.com");
...
test("with non falsy, but non url value", async () => {
    Product._fields.url.default = "tcrm://hello";
    await mountView({
        type: "form",
        resModel: "product",
        arch: `<form><field name="url" widget="url"/></form>`,
    });
    expect(".o_field_widget[name=url] a").toHaveAttribute("href", "http://tcrm://hello");
});
