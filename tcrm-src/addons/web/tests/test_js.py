...
class QunitCommon(tcrm.tests.HttpCase):
...
@tcrm.tests.tagged('post_install', '-at_install')
class HOOTCommon(tcrm.tests.HttpCase):
...
class WebSuite(QunitCommon, HOOTCommon):
    ...
    def _check_forbidden_statements(self, bundle):
        ...
        self.env.ref('web.layout').write({'arch_db': '<t t-name="web.layout"><html><head><meta charset="utf-8"/><link/><script id="web.layout.tcrmscript"/><meta/><t t-esc="head"/></head><body><t t-out="0"/></body></html></t>'})
