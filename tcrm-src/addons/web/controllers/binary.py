...
try:
    ...
    from tcrm.tools._vendor.send_file import send_file

import tcrm
import tcrm.modules.registry
from tcrm import SUPERUSER_ID, _, http, api
from tcrm.addons.base.models.assetsbundle import ANY_UNIQUE
from tcrm.exceptions import AccessError, UserError
from tcrm.http import request, Response
from tcrm.tools import file_open, file_path, replace_exceptions, str2bool
from tcrm.tools.image import image_guess_size_from_field_name
from tcrm.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Tcrm is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Tcrm is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if tcrm.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=tcrm.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or tcrm.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Tcrm logo instead", exc_info=True)
