...
from tcrm.http import content_disposition, dispatch_rpc, request, Response
from tcrm.service import db
from tcrm.tools.misc import file_open, str2bool
from tcrm.tools.translate import _

from tcrm.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = tcrm.tools.config.verify_admin_password('admin')
        d['list_db'] = tcrm.tools.config['list_db']
        d['langs'] = tcrm.service.db.exp_list_lang()
        d['countries'] = tcrm.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = tcrm.service.db.list_db_incompatible(d['databases'])
        except tcrm.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = tcrm.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            with tcrm.modules.registry.Registry(name).cursor() as cr:
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            tcrm.service.db.check_super(master_pwd)
            ...
            dump_stream = tcrm.service.db.dump_db(name, None, backup_format, filestore)
