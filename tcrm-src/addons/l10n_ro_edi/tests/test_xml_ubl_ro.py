...
from tcrm.tools import file_open

from tcrm.addons.l10n_account_edi_ubl_cii_tests.tests.common import (
    TestUBLCommon,
)
from tcrm.addons.l10n_ro_edi.models.account_move import HOLDING_DAYS
    ...
    answer_data = {
        ...
        '3030439533': {
            ...
            'invoice': {
                ...
                'attachment_raw': file_open("l10n_ro_edi/tests/test_files/from_tcrm/ciusro_in_invoice.xml").read(),
            },
        },
        '3029027563': {
...
class TestUBLRO(TestUBLCommon):
    ...
    def test_export_invoice(self):
        ...
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_tcrm/ciusro_out_invoice.xml')
    ...
    def test_export_credit_note(self):
        ...
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_tcrm/ciusro_out_refund.xml')

    def test_export_invoice_different_currency(self):
        invoice = self.create_move("out_invoice")
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_tcrm/ciusro_out_invoice_different_currency.xml')
    ...
    def test_export_invoice_without_country_code_prefix_in_vat(self):
        ...
        invoice = self.create_move("out_invoice", currency_id=self.company.currency_id.id)
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_tcrm/ciusro_out_invoice_no_prefix_vat.xml')
    ...
    def test_export_no_vat_but_have_company_registry_without_prefix(self):
        ...
        invoice = self.create_move("out_invoice", currency_id=self.company.currency_id.id)
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(attachment, xpaths=None, expected_file_path='from_tcrm/ciusro_out_invoice_no_prefix_company_registry.xml')
    ...
    @patch('tcrm.addons.l10n_ro_edi.models.account_move._request_ciusro_synchronize_invoices', new=_patch_request_ciusro_synchronize_invoices)
    ...
    def test_ciusro_synchronize_invoices_validation_error(self):
        ...
        self.assertEqual(str(last_message.body), '<p>Error when trying to download the E-Factura data from the SPV: There has been an connection error when downloading the message content</p>')
