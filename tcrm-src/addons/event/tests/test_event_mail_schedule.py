...
from tcrm.addons.event.tests.common import EventCase
from tcrm.addons.event.models.event_mail import EventMail
from tcrm.addons.mail.tests.common import MailCase
from tcrm.tests import tagged, users, warmup
...
class TestMailSchedule(EventMailCommon):
    ...
    @mute_logger('tcrm.addons.base.models.ir_model', 'tcrm.models')
    ...
    @mute_logger('tcrm.addons.event.models.event_mail')
    @users('user_eventmanager')
            ...
            [{
                ...
                'mail_mail_values': {
                    'body': f'<p>Communication for {self.test_event.name} scheduled on {before_scheduler.scheduled_date} failed. '
                            f'This may be linked to template <a href="{before_scheduler.get_base_url()}/tcrm/mail.template/{self.template_reminder.id}">'
    ...
    @mute_logger('tcrm.addons.event.models.event_mail', 'tcrm.addons.mail.models.mail_render_mixin')
    @users('user_eventmanager')
    def test_event_mail_schedule_fail_global_composer_message(self):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'mail_mail_values': {
                    'body': f'<p>Communication for {self.test_event.name} scheduled on {before_scheduler.scheduled_date} failed. '
                            f'This is due to an error in template <a href="{before_scheduler.get_base_url()}/tcrm/mail.template/{self.template_reminder.id}">'
    ...
    @mute_logger(
        'tcrm.addons.event.models.event_mail',
        'tcrm.addons.event.models.event_mail_registration',
        'tcrm.addons.event.models.event_registration',
    )
            ...
            [{
                ...
                'mail_mail_values': {
                    'body': f'<p>Communication for {self.test_event.name} scheduled on {test_dt} failed. '
                            f'This may be linked to template <a href="{onsub_scheduler.get_base_url()}/tcrm/mail.template/{onsub_scheduler.template_ref.id}">'
