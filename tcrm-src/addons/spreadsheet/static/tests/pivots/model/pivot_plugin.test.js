import { describe, expect, test, Deferred } from "@tcrm/hoot";
import { animationFrame, mockDate } from "@tcrm/hoot-mock";
import {
...
test("evaluates only once when two pivots are loading", async function () {
    ...
    model.config.custom.tcrmDataProvider.addEventListener("data-source-updated", () =>
...
test("can edit pivot domain with UPDATE_tcrm_PIVOT_DOMAIN", async () => {
    const { model } = await createSpreadsheetWithPivot();
    const [pivotId] = model.getters.getPivotIds();
