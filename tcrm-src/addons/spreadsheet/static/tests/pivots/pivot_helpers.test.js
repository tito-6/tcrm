...
import { constants, tokenize, helpers } from "@tcrm/o-spreadsheet";
...
test("Basic formula extractor", async function () {
    const formula = `=PIVOT.VALUE("1", "test") + TCRM.LIST("2", "hello", "bla")`;
    ...
    expect(functionName).toBe("TCRM.LIST");
...
test("Number of formulas", async function () {
    const formula = `=PIVOT.VALUE("1", "test") + PIVOT.VALUE("2", "hello", "bla") + TCRM.LIST("1", "bla")`;
