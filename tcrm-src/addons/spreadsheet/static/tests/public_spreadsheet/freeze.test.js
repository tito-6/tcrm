import { animationFrame, describe, expect, test } from "@tcrm/hoot";
import { registries } from "@tcrm/o-spreadsheet";
...
import { freezetcrmData, waitForDataLoaded } from "@spreadsheet/helpers/model";
import { tcrmPivot, tcrmPivotRuntimeDefinition } from "@spreadsheet/pivot/tcrm_pivot";
...
test("tcrm pivot functions are replaced with their value", async function () {
    ...
    const data = await freezetcrmData(model);
...
test("Pivot with a type different of TCRM is not converted", async function () {
    // Add a pivot with a type different of TCRM
...
test("tcrm pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezetcrmData(model);
...
test("tcrm charts are replaced with an image", async function () {
...
test("Carousels figure with tcrm data is converted to an image", async function () {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Empty TCRM.LIST result is frozen to an empty string", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", '=TCRM.LIST(1, 9999,"probability")'); // has no record
    await waitForDataLoaded(model);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    const frozenData = await freezetcrmData(model);
    expect(frozenData.sheets[0].cells.A1).toBe('=""');
});

test("tcrm links are replaced with their label", async function () {
    const view = {
        name: "an tcrm view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: "[menu_xml](tcrm://ir_menu_xml_id/test_menu)",
                    A2: "[menu_id](tcrm://ir_menu_id/12)",
                    A3: `[tcrm_view](tcrm://view/${JSON.stringify(view)})`,
                    A4: "[external_link](https://tcrm.com)",
    ...
    expect(frozenData.sheets[0].cells.A3).toBe("tcrm_view");
    expect(frozenData.sheets[0].cells.A4).toBe("[external_link](https://tcrm.com)");
