import { click } from "@tcrm/hoot-dom";
import { animationFrame } from "@tcrm/hoot-mock";
import { expect, test, beforeEach } from "@tcrm/hoot";
...
test("icon external link isn't on the chart when its not linked to an tcrm menu", async function () {
    ...
    await animationFrame();
    const tcrmMenu = model.getters.getCharttcrmMenu(chartId);
    expect(tcrmMenu).toBe(undefined, { message: "No menu linked with the chart" });

    const externalRefIcon = fixture.querySelector(".o-chart-external-link");
    expect(externalRefIcon).toBe(null);
});

test("icon external link is on the chart when its linked to an tcrm menu", async function () {
    const { model } = await createModelWithDataSource({
        serverData,
    });
    ...
    const chartMenu = model.getters.getCharttcrmMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "Tcrm menu is linked to chart" });
    await animationFrame();
    expect(".o-chart-external-link").toHaveCount(1);
});

test("icon external link is not on the chart when its linked to a wrong tcrm menu", async function () {
    const { model } = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
    ...
    model.dispatch("LINK_tcrm_MENU_TO_CHART", {
        chartId,
        tcrmMenuId: "menu which does not exist",
    });
    const chartMenu = model.getters.getCharttcrmMenu(chartId);
...
test("click on icon external link on chart redirect to the tcrm menu", async function () {
    ...
    const chartMenu = model.getters.getCharttcrmMenu(chartId);
    expect(chartMenu.id).toBe(2, { message: "Tcrm menu is linked to chart" });
    await animationFrame();
...
test("can use menus xmlIds instead of menu ids", async function () {
    ...
    model.dispatch("LINK_tcrm_MENU_TO_CHART", {
        chartId,
        tcrmMenuId: "spreadsheet.test.menu2",
    });
    await animationFrame();
...
test("Trying to open a menu without an action sends a notification to the user", async function () {
    ...
    model.dispatch("LINK_tcrm_MENU_TO_CHART", {
        chartId,
        tcrmMenuId: "spreadsheet.test.menu_without_action",
    });
    await animationFrame();
