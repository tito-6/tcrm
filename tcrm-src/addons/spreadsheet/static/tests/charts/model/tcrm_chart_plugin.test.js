...
import { tcrmBarChart } from "@spreadsheet/chart/odoo_chart/tcrm_bar_chart";
import { tcrmChart } from "@spreadsheet/chart/odoo_chart/tcrm_chart";
import { tcrmLineChart } from "@spreadsheet/chart/odoo_chart/tcrm_line_chart";
...
test("Can add an Tcrm Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof tcrmBarChart).toBe(true);
...
test("Can add an Tcrm Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof tcrmLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Tcrm Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof tcrmChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Tcrm bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "tcrm_bar",
...
test("Tcrm pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "tcrm_pie",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Tcrm line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "tcrm_line",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Data reloaded upon domain update for charts other than pie/bar/line", async () => {
    ...
    updateChart(model, chartId, { type: "tcrm_pie" });
    await waitForDataLoaded(model);
...
test("Can import/export an Tcrm chart", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "tcrm_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("tcrm_line");
...
test("Can undo/redo an Tcrm chart creation", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "tcrm_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_pie" });
    insertChartInSpreadsheet(model, "tcrm_bar");
    insertChartInSpreadsheet(model, "tcrm_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Tcrm chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof tcrmChart).toBe(true);
...
test("Can cut/paste Tcrm chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof tcrmChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Tcrm chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load tcrm chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert tcrm chart from a different model", async () => {
    const { model } = await createModelWithDataSource();
...
test("Tcrm chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove tcrm chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.gettcrmChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.gettcrmChartIds().length).toBe(0);
});

test("Tcrm chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.gettcrmChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.gettcrmChartDisplayName(chartId)).toBe("(#1) Tcrm Line Chart");
});
...
test("Tcrm line and bar charts display only horizontal grid lines", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "tcrm_line",
    });
    const sheetId = model.getters.getActiveSheetId();
...
test("Can configure the chart datasets", async () => {
    ...
    const { model } = await createSpreadsheetWithChart({
        type: "tcrm_bar",
        serverData,
        definition: { type: "tcrm_bar", metaData, searchParams, id: "42" },
    });
    await waitForDataLoaded(model);
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("Non-web chart types are using data source in 'bar' mode", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "tcrm_radar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
