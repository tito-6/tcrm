...
import { load } from "@tcrm/o-spreadsheet";
...
test("Tcrm formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
    ...
    expect(migratedData.sheets[0].cells.A3).toBe(`=TCRM.FILTER.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=TCRM.LIST("1")`);
    expect(migratedData.sheets[0].cells.A5).toBe(`=TCRM.LIST.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A6).toBe(`=TCRM.PIVOT.POSITION("1")`);
...
test("Pivot 'day' arguments are migrated", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=TCRM.PIVOT("1","21/07/2022")` },
                    A2: { content: `=TCRM.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=tcrm.pivot("1","21/07/2021")` },
                    A4: { content: `=TCRM.PIVOT("1","test")` },
                    A5: { content: `=tcrm.pivot("1","21/07/2021")+"21/07/2021"` },
...
test("Pivot formulas are migrated from 9 to 10", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=TCRM.PIVOT("1")` },
                    A2: { content: `=TCRM.PIVOT.HEADER("1")` },
                    A3: { content: `=TCRM.PIVOT.POSITION("1")` },
                    A4: { content: `=TCRM.PIVOT.TABLE("1")` },
                    A5: { content: `=tcrm.pivot("1")` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(`=PIVOT.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=TCRM.PIVOT.POSITION("1")`);
...
test("Pivot formulas using pivot positions are migrated (11 to 12)", () => {
        ...
        sheets: [
            {
                cells: {
                    A1: {
                        content: `=-PIVOT.VALUE("1","balance","account_id",TCRM.PIVOT.POSITION("1","account_id",12),"date:quarter","4/"&TCRM.FILTER.VALUE("Year"))`,
                    },
                    A2: {
                        content: `=PIVOT.HEADER("1","account_id",TCRM.PIVOT.POSITION("1","account_id",14))`,
                    },
                    A3: { content: `=TCRM.PIVOT.POSITION("1","account_id",14)` },
                    A4: { content: `=TCRM.PIVOT.POSITION("1",14)` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=-PIVOT.VALUE("1","balance","#account_id",12,"date:quarter","4/"&TCRM.FILTER.VALUE("Year"))`
    );
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1","#account_id",14)`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=TCRM.PIVOT.POSITION("1","account_id",14)`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=TCRM.PIVOT.POSITION("1",14)`);
});
