...
const MAP_V1 = {
    PIVOT: "TCRM.PIVOT",
    "PIVOT.HEADER": "TCRM.PIVOT.HEADER",
    "PIVOT.POSITION": "TCRM.PIVOT.POSITION",
    "FILTER.VALUE": "TCRM.FILTER.VALUE",
    LIST: "TCRM.LIST",
    "LIST.HEADER": "TCRM.LIST.HEADER",
};

const MAP_FN_NAMES_V10 = {
    "TCRM.PIVOT": "PIVOT.VALUE",
    "TCRM.PIVOT.HEADER": "PIVOT.HEADER",
    "TCRM.PIVOT.TABLE": "PIVOT",
};
...
migrationStepRegistry.add("17.3.1", {
    migrate(data) {
        return migratetcrmData(data);
    },
});
migrationStepRegistry.add("18.1.2", {
    migrate(data) {
        const version = data.tcrmVersion || 0;
        if (version < 13) {
    ...
    migrate(data) {
        for (const sheet of data.sheets || []) {
            for (const figure of sheet.figures || []) {
                if (
                    figure.tag === "chart" &&
                    ["tcrm_bar", "tcrm_line", "tcrm_pie"].includes(figure.data.type) &&
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["TCRM.PIVOT", "TCRM.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
/**
 * Migrate the pivot data to add the type, by default "TCRM". And replace the
...
function migrate11to12(data) {
    // remove the calls to TCRM.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("TCRM.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that tcrm.pivot.position is automatically set, we know that:
                1) it is always on the form of TCRM.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one tcrm.pivot.position per cell
                4) tcrm.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "TCRM.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "TCRM.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before tcrm.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
/**
 * Change pivot sortedColumn of the tcrm's pivot model to the sortedColumn of o-spreadsheet
 */
