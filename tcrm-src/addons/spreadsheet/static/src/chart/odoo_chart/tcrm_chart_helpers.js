...
export function ontcrmChartItemClick(getters, chart) {
    return navigateIntcrmMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onWaterfalltcrmChartItemClick(getters, chart) {
    return navigateIntcrmMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onGeotcrmChartItemClick(getters, chart) {
    return navigateIntcrmMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onSunbursttcrmChartItemClick(getters, chart) {
    return navigateIntcrmMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
...
export function onTreemaptcrmChartItemClick(getters, chart) {
    return navigateIntcrmMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
        const rawItem = chartJSChart.data.datasets[datasetIndex].data[index];
...
function navigateIntcrmMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items, chartJSChart) => {
        const env = getters.gettcrmEnv();
        const { datasets, labels } = chart.dataSource.getData();
...
export function ontcrmChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
...
export function onGeotcrmChartItemHover() {
    return (event, items) => {
        if (!items.length) {
            event.native.target.style.cursor = "";
            return;
...
export async function navigateTotcrmMenu(menu, actionService, notificationService, newWindow) {
    if (!menu) {
