...
export class tcrmChartFeaturePlugin extends tcrmUIPlugin {
    ...
    getAvailableChartGranularities(chartId) {
        ...
        const matching = this.getters.gettcrmChartFieldMatching(chartId, filterId);
    ...
    _getChartHorizontalAxisFilter(chartId, fieldName) {
        for (const filter of this.getters.getGlobalFilters()) {
            const matching = this.getters.gettcrmChartFieldMatching(chartId, filter.id);
