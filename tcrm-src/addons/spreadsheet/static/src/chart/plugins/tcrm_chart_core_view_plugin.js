...
export class tcrmChartCoreViewPlugin extends tcrmUIPlugin {
    static getters = /** @type {const} */ (["getChartDataSource", "gettcrmEnv"]);
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.gettcrmChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.gettcrmChartIds().includes(cmd.chartId)) {
                        continue;
                    }
                    if (this._shouldReloadDataSource(cmd.chartId, cmd.definition)) {
                        this._resetChartDataSource(cmd.chartId);
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._gettcrmChartDataSourceId(chartId);
    ...
    _gettcrmChartDataSourceId(chartId) {
    ...
    _refreshtcrmCharts() {
        for (const chartId of this.getters.gettcrmChartIds()) {
            this._refreshtcrmChart(chartId);
        }
    }
