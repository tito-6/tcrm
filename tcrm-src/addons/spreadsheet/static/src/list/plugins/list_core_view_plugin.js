...
import { tcrmCoreViewPlugin } from "@spreadsheet/plugins";
...
export class ListCoreViewPlugin extends tcrmCoreViewPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "SET_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_tcrm_LIST":
            case "UPDATE_tcrm_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const updateCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_tcrm_LIST_DOMAIN" ||
                        cmd.type === "UPDATE_tcrm_LIST" ||
                        cmd.type === "INSERT_tcrm_LIST"
                );
    ...
    getListFieldFromPosition(position) {
        ...
        const fieldArg = functionName === "TCRM.LIST.HEADER" ? args[1] : args[2];
