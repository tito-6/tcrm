...
export class ListCorePlugin extends tcrmCorePlugin {
    ...
    _insertHeaders(sheetId, anchor, id, columns) {
        ...
        for (const column of columns) {
            const content = column.string
                ? `=TCRM.LIST.HEADER(${id},"${column.name}","${column.string}")`
                : `=TCRM.LIST.HEADER(${id},"${column.name}")`;
        ...
        for (let i = 1; i <= linesNumber; i++) {
            ...
            for (const column of columns) {
                this.dispatch("UPDATE_CELL", {
                    ...
                    col,
                    row,
                    content: `=TCRM.LIST(${id},${i},"${column.name}")`,
                });
                col++;
            }
            row++;
