...
import { EvaluationError, helpers } from "@tcrm/o-spreadsheet";

const { isDateOrDatetimeField } = helpers;

/**
 * @typedef {import("@tcrm/o-spreadsheet").Token} Token
 * @typedef {import("@tcrm/o-spreadsheet").Granularity} Granularity
 * */
...
export const tcrm_AGGREGATORS = {};

for (const type in AGGREGATORS_BY_FIELD_TYPE) {
    tcrm_AGGREGATORS[type] = {};
    for (const aggregator of AGGREGATORS_BY_FIELD_TYPE[type]) {
        tcrm_AGGREGATORS[type][aggregator] = AGGREGATOR_NAMES[aggregator];
    }
}
...
/**
 * @typedef {import("@spreadsheet").tcrmField} tcrmField
 */

/**
 * Parses the positional char (#), the field and operator string of pivot group.
 * e.g. "create_date:month"
 * @param {Record<string, tcrmField | undefined>} allFields
 * @param {string} groupFieldString
 * @returns {{field: tcrmField, granularity: Granularity, isPositional: boolean, dimensionWithGranularity: string}}
 */
