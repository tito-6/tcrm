...
import { EvaluationError, CellErrorType } from "@tcrm/o-spreadsheet";
...
export class tcrmPivotLoader {
    /**
     * @param {tcrmDataProvider} tcrmDataProvider
     * @param {Function} load Function to fetch data
     */
    constructor(tcrmDataProvider, load) {
        /** @private @type {tcrmDataProvider} */
        this.tcrmDataProvider = tcrmDataProvider;
        /** @protected */
        this.loadFn = load;
    ...
    async load(options) {
        if (options && options.reload) {
            this.tcrmDataProvider.cancelPromise(this.loadPromise);
            this.loadPromise = undefined;
        }
        if (!this.loadPromise) {
            this.isFullyLoaded = false;
            ...
            await this.tcrmDataProvider.notifyWhenPromiseResolves(this.loadPromise);
        }
     ...
     * @returns {Promise<tcrmFields>} Fields of the model
     */
    async getFields(model) {
        return getFields(this.tcrmDataProvider.fieldService, model);
    }
    /**
     * @param {string} model Technical name of the model
    ...
    async getModelLabel(model) {
        const result = await this.tcrmDataProvider.orm
            .cache({ type: "disk" })
