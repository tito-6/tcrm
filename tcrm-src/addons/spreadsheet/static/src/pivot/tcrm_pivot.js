...
import { NO_RECORD_AT_THIS_POSITION, tcrmPivotModel } from "./pivot_model";
import { EvaluationError, PivotRuntimeDefinition, registries, helpers } from "@tcrm/o-spreadsheet";
import { LOADING_ERROR } from "@spreadsheet/data_sources/data_source";
import { omit } from "@web/core/utils/objects";
import { tcrmPivotLoader } from "./tcrm_pivot_loader";
...
/**
 * @typedef {import("@tcrm/o-spreadsheet").FunctionResultObject} FunctionResultObject
 * @typedef {import("@tcrm/o-spreadsheet").PivotMeasure} PivotMeasure
 * @typedef {import("@tcrm/o-spreadsheet").PivotDomain} PivotDomain
 * @typedef {import("@tcrm/o-spreadsheet").PivotDimension} PivotDimension
 * @typedef {import("@tcrm/o-spreadsheet").PivotCoreMeasure} PivotCoreMeasure
 * @typedef {import("@spreadsheet").WebPivotModelParams} WebPivotModelParams
 * @typedef {import("@spreadsheet").tcrmPivot<tcrmPivotRuntimeDefinition>} IPivot
 * @typedef {import("@spreadsheet").tcrmFields} tcrmFields
 * @typedef {import("@spreadsheet").tcrmPivotCoreDefinition} tcrmPivotCoreDefinition
 * @typedef {import("@spreadsheet").tcrmGetters} tcrmGetters
 * @typedef {import("@spreadsheet/data_sources/tcrm_data_provider").tcrmDataProvider} tcrmDataProvider
 */

/**
 * @implements {IPivot}
 */
export class tcrmPivot {
    /**
     *
     * @override
     * @param {Object} services Services (see DataSource)
     * @param {Object} params
     * @param {tcrmPivotCoreDefinition} params.definition
     * @param {tcrmGetters} params.getters
     */
    constructor(services, { definition, getters }) {
        /** @type {"TCRM"} */
        this.type = "TCRM";

        /** @type {tcrmPivotCoreDefinition} @protected */
        this.coreDefinition = definition;

        this.needsReevaluation = false;

        /** @type {tcrmPivotRuntimeDefinition | undefined} @protected */
        this.runtimeDefinition = undefined;

        /** @type {tcrmPivotModel | undefined} @protected */
        this.model = undefined;

        /** @type {tcrmGetters} @protected */
        this.getters = getters;

        /** @protected */
        this.loader = new tcrmPivotLoader(services.tcrmDataProvider, this._load.bind(this));

        /** @type {tcrmFields | undefined} @protected */
        this._fields = undefined;

        /** @protected @type {tcrmDataProvider}*/
        this.tcrmDataProvider = services.tcrmDataProvider;
    ...
    /**
     * @param {tcrmPivotCoreDefinition} nextDefinition
     */
    onDefinitionChange(nextDefinition) {
        ...
        if (
            ...
            if (
                ...
                this.coreDefinition = nextDefinition;
                const runtimeDefinition = new tcrmPivotRuntimeDefinition(
                    this.coreDefinition,
                    this.getFields()
                );
    ...
    async createModelAndDefinition() {
        ...
        const definition = new tcrmPivotRuntimeDefinition(this.coreDefinition, this.getFields());
        const model = new tcrmPivotModel(
            { _t },
            {
            ...
            {
                orm: this.tcrmDataProvider.orm,
                serverData: this.tcrmDataProvider.serverData,
    ...
    /**
     * @param {import("@tcrm/o-spreadsheet").Maybe<FunctionResultObject>[]} args
     *
     * @returns {PivotDomain}
     */
        ...
        await Promise.all(
            related.map((dimension) =>
                getRelationalFieldDefinition(
                    ...
                    dimension.fieldName,
                    this.tcrmDataProvider.fieldService
    ...
    async _loadPropertiesDefinitions() {
        ...
        const orm = this.tcrmDataProvider.orm;
...
export class tcrmPivotRuntimeDefinition extends PivotRuntimeDefinition {
    /**
     * @param {tcrmPivotCoreDefinition} definition
     * @param {tcrmFields} fields
     */
...
supportedPivotPositionalFormulaRegistry.add("TCRM", true);
