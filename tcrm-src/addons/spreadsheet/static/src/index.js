...
import {
    CharttcrmMenuPlugin,
    tcrmChartCorePlugin,
    tcrmChartCoreViewPlugin,
} from "@spreadsheet/chart/index"; // tcrmchart depends on filter for its getters
import { PivotCoreGlobalFilterPlugin } from "./pivot/plugins/pivot_core_global_filter_plugin";
import { PivottcrmUIPlugin } from "./pivot/plugins/pivot_tcrm_ui_plugin";
...
import { tcrmChartFeaturePlugin } from "./chart/plugins/tcrm_chart_feature_plugin";
...
globalFieldMatchingRegistry.add("pivot", {
    getIds: (getters) =>
        getters
            ...
            .filter(
                (id) =>
                    getters.getPivotCoreDefinition(id).type === "TCRM" &&
    ...
    getModel: (getters, pivotId) => {
        ...
        return pivot.type === "TCRM" && pivot.model;
    },
    waitForReady: (getters) =>
        getters
            .getPivotIds()
            .map((pivotId) => getters.getPivot(pivotId))
            .filter((pivot) => pivot.type === "TCRM")
...
globalFieldMatchingRegistry.add("chart", {
    getIds: (getters) => getters.gettcrmChartIds(),
    getDisplayName: (getters, chartId) => getters.gettcrmChartDisplayName(chartId),
    getFieldMatching: (getters, chartId, filterId) =>
        getters.gettcrmChartFieldMatching(chartId, filterId),
...
corePluginRegistry.add("tcrmGlobalFiltersCorePlugin", GlobalFiltersCorePlugin);
corePluginRegistry.add("PivottcrmCorePlugin", PivottcrmCorePlugin);
corePluginRegistry.add("tcrmPivotGlobalFiltersCorePlugin", PivotCoreGlobalFilterPlugin);
corePluginRegistry.add("tcrmListCorePlugin", ListCorePlugin);
corePluginRegistry.add("tcrmListCoreGlobalFilterPlugin", ListCoreGlobalFilterPlugin);
corePluginRegistry.add("tcrmChartCorePlugin", tcrmChartCorePlugin);
corePluginRegistry.add("charttcrmMenuPlugin", CharttcrmMenuPlugin);

coreViewsPluginRegistry.add("tcrmGlobalFiltersCoreViewPlugin", GlobalFiltersCoreViewPlugin);
coreViewsPluginRegistry.add(
    "tcrmPivotGlobalFiltersCoreViewPlugin",
    PivotCoreViewGlobalFilterPlugin
);
coreViewsPluginRegistry.add("tcrmListCoreViewPlugin", ListCoreViewPlugin);
coreViewsPluginRegistry.add("tcrmChartCoreViewPlugin", tcrmChartCoreViewPlugin);

featurePluginRegistry.add("tcrmPivotGlobalFilterUIPlugin", PivotUIGlobalFilterPlugin);
featurePluginRegistry.add("tcrmGlobalFiltersUIPlugin", GlobalFiltersUIPlugin);
featurePluginRegistry.add("tcrmPivotUIPlugin", PivottcrmUIPlugin);
featurePluginRegistry.add("tcrmListUIPlugin", ListUIPlugin);
featurePluginRegistry.add("tcrmChartFeaturePlugin", tcrmChartFeaturePlugin);
