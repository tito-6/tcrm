import { EvaluationError, helpers, registries } from "@tcrm/o-spreadsheet";
...
export class CurrencyPlugin extends tcrmUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.tcrmDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'tcrmDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("tcrmCurrency", CurrencyPlugin);
