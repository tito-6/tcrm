...
from tcrm.tools.view_validation import get_domain_value_names


markdown_link_regex = r"^\[([^\[]+)\]\((.+)\)$"

xml_id_url_prefix = "tcrm://ir_menu_xml_id/"

tcrm_view_link_prefix = "tcrm://view/"


def tcrm_charts(data):
    """returns all tcrm chart definitions in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        for figure in sheet.get("figures", []):
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("tcrm_"):
                figures.append(dict(figure["data"], id=figure["id"]))
            elif figure["tag"] == "carousel":
                figures.extend(get_tcrm_charts_from_carousel(figure["data"]))
    return figures


def get_tcrm_charts_from_carousel(carousel):
    charts = []
    for chart_id, chart in carousel["chartDefinitions"].items():
        if chart["type"].startswith("tcrm_"):
...
def tcrm_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like tcrm://view/{... view data...}
    """
    return [
        json.loads(url[len(tcrm_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(tcrm_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = tcrm_charts(data)
    if "tcrmVersion" in data and data["tcrmVersion"] < 5:
...
def fields_in_spreadsheet(data):
    ...
    tcrm_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "TCRM") == "TCRM")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, tcrm_pivots).items(),
        extract_fields(chart_fields, tcrm_charts(data)).items(),
        extract_fields(tcrm_view_fields, tcrm_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("charttcrmMenusReferences", {}).values()) | {
