...
from tcrm.addons.spreadsheet.utils.formatting import (
...
class ResLang(models.Model):
    ...
    def get_locales_for_spreadsheet(self):
        ...
        spreadsheet_locales = [lang._tcrm_lang_to_spreadsheet_locale() for lang in langs]
        return spreadsheet_locales

    @api.model
    def _get_user_spreadsheet_locale(self):
        """Convert the tcrm lang to a spreadsheet locale."""
        lang = self._lang_get(self.env.user.lang or 'en_US')
        return lang._tcrm_lang_to_spreadsheet_locale()

    def _tcrm_lang_to_spreadsheet_locale(self):
        """Convert an tcrm lang to a spreadsheet locale."""
        return {
            "name": self.name,
            "code": self.code,
