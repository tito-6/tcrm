...
from tcrm.addons.im_livechat.tests.common import TestImLivechatCommon
from tcrm.tests.common import users, tagged
...
class TestImLivechatSessionViews(TestImLivechatCommon):
    def test_session_history_navigation_back_and_forth(self):
        ...
        self.start_tour(
            f"/tcrm/livechat/{self.livechat_channel.id}/action-{action.id}",
...
class TestImLivechatLookingForHelpViews(TestImLivechatSessionViews):
    ...
    def test_looking_for_help_list_real_time_update(self):
        ...
        self.start_tour(
            f"/tcrm/action-{self.looking_for_help_action.id}",
    ...
    def test_looking_for_help_kanban_real_time_update(self):
        self.start_needhelp_session()
        self.start_tour(
            f"/tcrm/action-{self.looking_for_help_action.id}?view_type=kanban",
    ...
    def test_looking_for_help_discuss_category(self):
        ...
        self._reset_bus()
        self.start_tour(
            "/tcrm/discuss", "im_livechat.looking_for_help_discuss_category_tour", login="agent"
        )
