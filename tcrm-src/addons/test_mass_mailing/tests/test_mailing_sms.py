...
from tcrm.addons.phone_validation.tools import phone_validation
from tcrm.addons.sms_twilio.tests.common import MockSmsTwilioApi
from tcrm.addons.test_mass_mailing.tests.common import TestMassSMSCommon
...
class TestMassSMS(TestMassSMSCommon):
    ...
    def test_mass_sms_links(self):
        mailing = self.env['mailing.mailing'].browse(self.mailing_sms.ids)
        mailing.write({
            'body_plaintext': 'Dear {{ object.display_name }} this is a mass SMS with two links http://www.tcrm.com/smstest and http://www.tcrm.com/smstest/{{ object.name }}',
        ...
        self.assertSMSTraces(
            ...
            mailing, self.records,
            sms_links_info=[[
                ('http://www.tcrm.com/smstest', True, {}),
                ('http://www.tcrm.com/smstest/%s' % record.name, True, {}),
    ...
    def test_mass_sms_partner_only(self):
        ...
        self.assertSMSTraces(
            ...
            sms_links_info=[[
                ('http://www.tcrm.com/smstest', True, {}),
                ('http://www.tcrm.com/smstest/%s' % record.id, True, {}),
                # unsubscribe is not shortened and parsed at sending
                ('unsubscribe', False, {}),
            ] for record in records],
        )
        ...
        self.assertSMSTraces(
            ...
            sms_links_info=[[
                ('http://www.tcrm.com/smstest', True, {}),
                ('http://www.tcrm.com/smstest/%s' % new_record.id, True, {}),
                # unsubscribe is not shortened and parsed at sending
                ('unsubscribe', False, {}),
            ]],
        )
