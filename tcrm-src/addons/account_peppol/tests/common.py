...
from tcrm.tools import DotDict
...
class PeppolConnectorCommon(AccountTestInvoicingCommon):
    ...
    def _mock_create_user(self, success=True):
        ...
        return (
            'https://peppol.test.tcrm.com/iap/account_edi/2/create_user',
            replacement_method,
        )

    def _mock_register_sender(self, success=True):
        return (
            'https://peppol.test.tcrm.com/api/peppol/1/register_sender',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

    def _mock_register_sender_as_receiver(self, success=True):
        return (
            'https://peppol.test.tcrm.com/api/peppol/1/register_sender_as_receiver',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )
    ...
    def _mock_lookup_participant(self, success=True):
        ...
        return (
            'https://peppol.test.tcrm.com/api/peppol/1/lookup',
            replacement_method,
        )
    ...
    def _mock_participant_status(self, peppol_state):
        ...
        return (
            'https://peppol.test.tcrm.com/api/peppol/2/participant_status',
            replacement_method,
        )

    def _mock_cancel_peppol_registration(self, success=True):
        return (
            'https://peppol.test.tcrm.com/api/peppol/1/cancel_peppol_registration',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

    def _mock_get_all_documents(self, success=True):
        return (
            'https://peppol.test.tcrm.com/api/peppol/1/get_all_documents',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )

    def _mock_update_user(self, success=True):
        return (
            'https://peppol.test.tcrm.com/api/peppol/1/update_user',
            lambda url, **kwargs: self._empty_result_or_error(success=success),
        )
