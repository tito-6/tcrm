...
import tcrm.exceptions
from tcrm.fields import Command, Date, Datetime
from tcrm.http import Controller, Response, dispatch_rpc, request, route
from tcrm.tools import lazy
from tcrm.tools.misc import frozendict
...
# User code must use the exceptions defined in ``tcrm.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, tcrm.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, tcrm.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, tcrm.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, tcrm.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, tcrm.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(tcrm.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class tcrmMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, tcrm is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = tcrmMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
