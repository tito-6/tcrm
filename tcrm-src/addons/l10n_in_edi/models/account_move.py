...
from tcrm import Command, _, api, fields, models
from tcrm.exceptions import AccessError, LockError, UserError
from tcrm.tools import float_is_zero, float_compare

from tcrm.addons.l10n_in.models.account_invoice import EDI_CANCEL_REASON
...
class AccountMove(models.Model):
    ...
    def _l10n_in_edi_send_invoice(self):
        ...
        if error := response.get('error', {}):
            tcrmbot_id = self.env.ref('base.partner_root').id
    ...
    def _l10n_in_edi_cancel_invoice(self):
        ...
        # Creating a lambda function so it fetches the tcrmbot id only when needed
        _get_tcrmbot_id = (
        ...
        if error := response.get('error'):
            ...
            if '9999' in error_codes:
                ...
                self.message_post(
                    author_id=_get_tcrmbot_id(self),
                    body=_(
        ...
        if "error" not in response:
            ...
            self.message_post(author_id=_get_tcrmbot_id(self), body=_(
    ...
    def _l10n_in_edi_connect_to_server(self, url_end_point, json_payload=False, params=False):
        ...
        if (error := response.get('error')) and '1005' in [e.get("code") for e in error]:
            ...
            # This happen when authenticate called from another tcrm instance with same credentials (like. Demo/Test)
