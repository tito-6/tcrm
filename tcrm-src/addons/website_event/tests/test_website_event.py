...
from tcrm.addons.website_event.tests.common import TestEventOnlineCommon, OnlineEventCase
...
class TestEventRegisterUTM(HttpCase, TestEventOnlineCommon):
    def test_event_registration_utm_values(self):
        ...
        self.opener.cookies.update({
            'tcrm_utm_campaign': event_campaign.name,
            'tcrm_utm_source': self.env.ref('utm.utm_source_newsletter').name,
            'tcrm_utm_medium': self.env.ref('utm.utm_medium_email').name
        })
