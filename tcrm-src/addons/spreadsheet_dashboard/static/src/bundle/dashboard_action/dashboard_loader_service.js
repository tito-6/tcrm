 ...
 *
 * @typedef {import("@web/env").tcrmEnv} tcrmEnv
 *
...
export class DashboardLoader {
    ...
    _createSpreadsheetModel(snapshot, revisions = [], currency, translationNamespace) {
        // NB: the local variable must NOT be named `tcrmDataProvider`: that is the
        // name of the imported class (see top of file). Reusing the same identifier
        // for a `const` shadows the import for the whole function scope and makes the
        // `new ...` call read the binding inside its own initializer, triggering a
        // temporal-dead-zone "Cannot access 'tcrmDataProvider' before initialization"
        // ReferenceError. Keep the class name for `new`, use a distinct local var.
        const dataProvider = new tcrmDataProvider(this.env);
        ...
        dataProvider.addEventListener("data-source-updated", () =>
