...
test("regression: dashboard model is created without tcrmDataProvider TDZ error", async () => {
    // Guards against a rebrand name-collision where the local variable in
    // DashboardLoader._createSpreadsheetModel was named `tcrmDataProvider`, the
    // same identifier as the imported class, causing a temporal-dead-zone
    // "Cannot access 'tcrmDataProvider' before initialization" ReferenceError.
    const loader = await createDashboardLoader();
    await loader.load();
    const result = loader.getDashboard(3);
    await animationFrame();
    ...
    expect(result.model).not.toBe(undefined);
    expect(result.model.config.custom.tcrmDataProvider).not.toBe(undefined);
});
        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: '=TCRM.CURRENCY.RATE("EUR","USD")', // an async formula
                },
            },
        ],
    };
