...
from tcrm.tests import HttpCase, get_db_name, tagged
from tcrm.tools import mute_logger

from tcrm.addons.auth_totp.tests.test_totp import TestTOTPMixin
...
class TestAPIKeys(TestTOTPMixin, HttpCase):
    ...
    def test_addremove(self):
        db = get_db_name()
        self.start_tour('/tcrm', 'apikeys_tour_setup', login=self.user_test.login)
        ...
        self.start_tour('/tcrm', 'apikeys_tour_teardown', login=self.user_test.login)

    def test_apikeys_totp(self):
        db = get_db_name()
        self.install_totphook()
        self.start_tour('/tcrm', 'apikeys_tour_setup', login=self.user_test.login)
        self.start_tour('/tcrm', 'totp_tour_setup', login=self.user_test.login)
