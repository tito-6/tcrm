...
from tcrm import exceptions
from tcrm.addons.mail.models.mail_message import MailMessage
from tcrm.addons.mail.models.mail_thread import MailThread
from tcrm.addons.mail.tests.common import mail_new_test_user, MailCommon
from tcrm.addons.test_mail.data import test_mail_data
from tcrm.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE, MAIL_TEMPLATE_EXTRA_HTML, THAI_EMAIL_WINDOWS_874
from tcrm.addons.test_mail.models.mail_test_ticket import MailTestTicket
from tcrm.addons.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups
from tcrm.sql_db import Cursor
from tcrm.tests import Form, tagged, RecordCapturer
from tcrm.tools import mute_logger
from tcrm.tools.mail import email_normalize, email_split_and_format, formataddr
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('tcrm.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:tcrm.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_eml(self):
        ...
        mail = self.format(test_mail_data.MAIL_EML_ATTACHMENT, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>', to=f'generic@{self.alias_domain}',
                           msg_id='<cb7eaf62-58dc-2017-148c-305d0c78892f@tcrm.com>',
                           references='<f3b9f8f8-28fa-2543-cab2-7aa68f679ebb@tcrm.com>',
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        incoming_bounce = self.format(
            ...
            email_from='MAILER-DAEMON@mailserver.tcrm.com (Mail Delivery System)',
            email_to='bounce@xxx.tcrm.com',
            delivered_to='bounce@xxx.tcrm.com'
        )
...
class TestMailgateway(MailGatewayCommon):
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.models')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not tcrmbot:
        ...
        tcrmbot.active = True
        tcrmbot.email = 'tcrmbot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, tcrmbot.email_formatted, f'groups@{self.alias_domain}',
                subject='tcrmbot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, tcrmbot)
        self.assertEqual(record4.message_ids[0].email_from, tcrmbot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: tcrmbot -> no follower')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.addons.mail.models.mail_thread', 'tcrm.models.unlink', 'tcrm.tests')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.addons.mail.models.mail_thread', 'tcrm.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on tcrmbot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'tcrmbot should be creating the message')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.models.unlink', 'tcrm.addons.mail.models.mail_mail')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.addons.mail.models.mail_mail', 'tcrm.models.unlink')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.addons.mail.models.mail_mail', 'tcrm.models', 'tcrm.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('tcrm.addons.mail.models.mail_alias.MailAlias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('tcrm.addons.mail.models.mail_alias.MailAlias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.models', 'tcrm.tests')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.models.unlink')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_thread', 'tcrm.models.unlink', 'tcrm.addons.mail.models.mail_mail', 'tcrm.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by tcrmbot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but tcrmbot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by tcrm should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        tcrm should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        ...
        # Tcrm 10 update: compat mode has been removed and should not work anymore
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Tcrm is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.addons.mail.models.mail_thread')
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Tcrm servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Tcrm server -> msg_id is
...
class TestMailGatewayReplies(MailGatewayCommon):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{email.message_id} {log.message_id} {reply.message_id}',  # should contain reference to tcrmExternal message, logs to fill up history
                },
                'mail_mail_values': {
    ...
    def test_routing_reply_internal_messages(self):
        """ Test routing notably between two tcrms when internal messages
        ...
        RFQ-like                                        creation log
                                                        initial_msg
        tcrm2 replies           creation log
                                reply                   reply
        -some internal work-                            user_notification
        tcrm1 replies           reply_2                 reply_2 (incoming email)
        -some internal work-                            log
        tcrm2 replies           reply_3                 reply_3 (outgoing email)

        Purpose: have references from tcrm2 containing message IDs to try to
        ...
        gateway_record.message_post(
            author_id=self.env.ref('base.partner_root').id,
            body='tcrmExternal Inquiry',
            email_from=self.partner_1.email_normalized,
            message_type='comment',
            subtype_id=self.env.ref('mail.mt_comment').id,
        )
        self.assertEqual(gateway_record.message_partner_ids, self.partner_admin)
        log, tcrmext_msg = gateway_record.message_ids[1], gateway_record.message_ids[0]
        self.assertEqual(tcrmext_msg.parent_id, log, 'Log serves as thread ancestor')
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{log.message_id} {tcrmext_msg.message_id} {reply.message_id}',  # should contain reference to tcrmExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
                    'parent_id': tcrmext_msg,  # attached to last comment / email when possible
                },
                'notif': [
                    {'partner': self.partner_1, 'type': 'email',},
        ...
        # coming from tcrm1: their reply as an incoming email
        with self.mock_mail_gateway():
            self.format_and_process(
                MAIL_TEMPLATE, self.email_from, reply.reply_to,
                subject='Gateway Creation',
                date=datetime.now(),
                extra=f'References: {reply.message_id} <msg1@tcrm1>',
                debug_log=True,
            )
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{log.message_id} {tcrmext_msg.message_id} {reply.message_id} {reply_2.message_id}',  # should contain reference to tcrmExternal message
                },
                'mail_mail_values': {
        ...
        self.assertMailNotifications(
            reply_3,
            [{
                'content': 'Tcrm Reply 2',
                'email_values': {
                    'message_id': reply_3.message_id,
                    'references': f'{tcrmext_msg.message_id} {reply.message_id} {reply_2.message_id} {reply_3.message_id}',  # should contain reference to tcrmExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
