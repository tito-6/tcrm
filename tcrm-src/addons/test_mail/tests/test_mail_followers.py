...
from tcrm.addons.mail.models.mail_mail import _UNFOLLOW_REGEX
...
class RecipientsNotificationTest(MailCommon):
    ...
    def test_subscribe_post_author(self):
        """ Test author is added in followers, unless it is archived / tcrmbot """
        # some automated action post on behalf of author
        test_record = self.env['mail.test.simple'].create({'name': 'Test'})
        self.partner_root.active = True  # edge case, people activating tcrmbot partner (not user)
        ...
        for user, author, exp_followers in [
            ...
            (self.user_2, self.user_root.partner_id, self.env['res.partner']),  # no tcrmbot !
        ]:
...
class UnfollowLinkTest(MailCommon, HttpCase):
    ...
    @mute_logger('tcrm.addons.base.models', 'tcrm.addons.mail.controllers.mail', 'tcrm.http', 'tcrm.models')
