...
from tcrm.tools.misc import mute_logger
from tcrm.addons.mail.tests.common import mail_new_test_user
from tcrm.addons.mail.tools.jwt import InvalidVapidError
from tcrm.addons.mail.tools.web_push import ENCRYPTION_BLOCK_OVERHEAD, ENCRYPTION_HEADER_SIZE
from tcrm.addons.sms.tests.common import SMSCommon
from tcrm.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE
...
class TestWebPushNotification(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['mail.push.device'].sudo().create([
            {
                'endpoint': f'https://test.tcrm.com/webpush/user{(idx + 1)}',
    ...
    @patch.object(tcrm.addons.mail.models.mail_thread, 'push_to_end_point')
    @mute_logger('tcrm.tests')
    def test_notify_by_push(self, push_to_end_point):
        """ When posting a comment, notify both inbox and people outside of Tcrm
        aka email """
        ...
        for channel, sender, notification_count in zip(
            ...
            with self.subTest(channel_type=channel.channel_type):
                ...
                if notification_count > 0:
                    ...
                    icon = (
                        '/web/static/img/tcrm-icon-192x192.png'
                        if sender == self.guest
                    ...
                    self.assertEqual(push_to_end_point.call_args.kwargs['device']['endpoint'], 'https://test.tcrm.com/webpush/user2')
    ...
    def test_notify_by_push_channel_with_channel_notifications_settings(self, push_to_end_point):
        ...
        self.env["mail.push.device"].sudo().create(
            [
                {
                    "endpoint": f"https://test.tcrm.com/webpush/user{(idx + 20)}",
                    "expiration_time": None,
                    "keys": json.dumps(
                        {
                            "p256dh": "BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A",
        ...
        self.assertEqual(push_to_end_point.call_args.kwargs["device"]["endpoint"], "https://test.tcrm.com/webpush/user20")
        push_to_end_point.reset_mock()
        ...
        self.assertEqual(push_to_end_point.call_args_list[0].kwargs["device"]["endpoint"], "https://test.tcrm.com/webpush/user20")
        self.assertEqual(push_to_end_point.call_args_list[1].kwargs["device"]["endpoint"], "https://test.tcrm.com/webpush/user21")
        push_to_end_point.reset_mock()
    ...
    def test_notify_by_push_mail_gateway(self):
        ...
        for include_as_external, has_notif in ((False, True), (True, False)):
            ...
            if has_notif:
                # user_inbox is notified by Tcrm, hence receives a push notification
        ...
        for recipient, has_notification in [(self.user_email, False), (self.user_inbox, True)]:
            with self.subTest(recipient=recipient):
                ...
                if has_notification:
                    self.assertPushNotification(
                        mail_push_count=0,
                        endpoint='https://test.tcrm.com/webpush/user2', keys=('vapid_private_key', 'vapid_public_key'),
    ...
    @patch.object(tcrm.addons.mail.models.mail_push, 'push_to_end_point')
    def test_push_notifications_cron(self, push_to_end_point):
        # Add 4 more devices to force sending via cron queue
        for index in range(10, 14):
            self.env['mail.push.device'].sudo().create([{
                'endpoint': 'https://test.tcrm.com/webpush/user%d' % index,
                'expiration_time': None,
                'keys': json.dumps({
                    'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                    'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    @patch.object(tcrm.addons.mail.models.mail_thread.Session, 'post',
    ...
    def test_push_notifications_error_device_unreachable(self, post):
        with mute_logger('tcrm.addons.mail.tools.web_push'):
            self.record_simple.with_user(self.user_email).message_notify(
                partner_ids=self.user_inbox.partner_id.ids,
                body='Test message send via Web Push',
                subject='Test Activity',
        ...
        notification_count = self.env['mail.push.device'].search_count([('endpoint', '=', 'https://test.tcrm.com/webpush/user2')])
    ...
    def test_push_notifications_error_encryption_simple(self, post):
        ...
        self._assert_notification_count_for_cron(0)
        post.assert_called_once()
        self.assertEqual(post.call_args.args[0], 'https://test.tcrm.com/webpush/user2')
    ...
    def test_push_notifications_device_invalid_tld_domain(self, post):
        self.env['mail.push.device'].sudo().create([{
            'endpoint': 'https://test.tcrm.invalid/webpush/user',
            'expiration_time': None,
            'keys': json.dumps({
                'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                'auth': 'DJFdtAgZwrT6yYkUMgUqow'
        ...
        device_count = self.env['mail.push.device'].search_count([('endpoint', '=', 'https://test.tcrm.invalid/webpush/user')])
    ...
    @patch.object(tcrm.addons.mail.models.mail_thread.Session, 'post', side_effect=ConnectionError("Oops, network error"))
    def test_push_notifications_device_raise_exception(self, post):
        ...
        with self.assertLogs('tcrm.addons.mail.models.mail_push', level="ERROR") as capture:
            self._assert_notification_count_for_cron(5)
            self._trigger_cron_job()
            self.assertEqual(capture.output, [
                'ERROR:tcrm.addons.mail.models.mail_push:An error occurred while trying to send web push: Oops, network error',
            ] * 5)
    ...
    def test_push_notification_regenerate_vapid_keys(self):
        ...
        with self.assertRaises(InvalidVapidError):
            self.env['mail.push.device'].register_devices(
                endpoint='https://test.tcrm.com/webpush/user1',
                expiration_time=None,
                keys=json.dumps({
                    'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                    'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    @patch.object(
        tcrm.addons.mail.models.mail_thread.Session, 'post', return_value=SimpleNamespace(status_code=201, text='Ok')
    )
    @patch.object(
        tcrm.addons.mail.models.mail_thread, 'push_to_end_point',
        wraps=tcrm.addons.mail.tools.web_push.push_to_end_point,
    )
    ...
    @patch.object(
        tcrm.addons.mail.tools.web_push, '_encrypt_payload',
        wraps=tcrm.addons.mail.tools.web_push._encrypt_payload,
    )
    def test_push_notifications_truncate_payload_mocked_size_limit(self, web_push_encrypt_payload_mock, thread_push_mock, session_post_mock):
        ...
        for size_limit, expected_body in [
            ...
            with self.subTest(size_limit=size_limit), patch.object(
                tcrm.addons.mail.models.mail_thread.MailThread, '_truncate_payload_get_max_payload_length',
