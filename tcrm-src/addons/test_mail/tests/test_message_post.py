...
from tcrm.addons.base.tests.test_ir_cron import CronMixinCase
from tcrm.addons.mail.tests.common import mail_new_test_user, MailCommon
from tcrm.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE_PLAINTEXT
from tcrm.addons.test_mail.models.test_mail_models import MailTestSimple
...
from tcrm.tools import mute_logger, formataddr
from tcrm.tests.common import users
...
class TestMailNotifyAPI(TestMessagePostCommon):
    ...
    @mute_logger('tcrm.models.unlink')
    @users('employee')
...
class TestMessageNotify(TestMessagePostCommon):
    ...
    @users('employee')
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.models.unlink', 'tcrm.tests')
...
class TestMessagePost(TestMessagePostCommon, CronMixinCase):
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.models.unlink')
    @users('employee')
    def test_message_post(self):
        ...
        exp_headers = {
            ...
            'X-Tcrm-Objects': f'{test_record._name}-{test_record.id}',
        }
        with self.assertSinglePostNotifications(
                [
                    {'partner': self.partner_employee_2, 'type': 'inbox'},
    ...
    def test_message_post_inactive_follower(self):
        """ Test posting with inactive followers does not notify them (e.g. tcrmbot) """
        test_record = self.env['mail.test.simple'].browse(self.test_record.ids)
                ...
                with self.assertSinglePostNotifications(
                        ...
                        message_info={
                            ...
                            'mail_mail_values': {
                                'headers': {
                                    'Return-Path': f'{expected_alias_domain.bounce_alias}@{expected_alias_domain.name}',
                                    'X-Tcrm-Objects': f'{record._name}-{record.id}',
                                },
                            },
                            'message_values': {
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.tests')
    ...
    @users('employee')
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.addons.mail.models.mail_message_schedule', 'tcrm.models.unlink')
    ...
    @mute_logger('tcrm.addons.mail.models.mail_mail', 'tcrm.addons.mail.models.mail_message_schedule')
    ...
    @mute_logger('tcrm.addons.base.models.ir_model', 'tcrm.addons.mail.models.mail_mail')
