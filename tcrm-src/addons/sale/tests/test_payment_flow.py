...
from tcrm.tests import JsonRpcException, tagged
from tcrm.tools import mute_logger

from tcrm.addons.account_payment.tests.common import AccountPaymentCommon
from tcrm.addons.http_routing.tests.common import MockRequest
from tcrm.addons.mail.tests.common import MailCase
from tcrm.addons.payment.tests.http_common import PaymentHttpCommon
from tcrm.addons.sale.controllers.portal import CustomerPortal
from tcrm.addons.sale.tests.common import SaleCommon
...
class TestSalePayment(AccountPaymentCommon, MailCase, PaymentHttpCommon, SaleCommon):
    ...
    @mute_logger('tcrm.http', 'werkzeug')
    ...
    def test_auto_confirm_and_auto_invoice(self):
        ...
        tx = self._create_transaction(flow='redirect', sale_order_ids=[self.sale_order.id], state='done')
        with (
            mute_logger('tcrm.addons.sale.models.payment_transaction'),
    ...
    def test_auto_done_and_auto_invoice(self):
        ...
        self.amount = self.sale_order.amount_total
        tx = self._create_transaction(flow='redirect', sale_order_ids=[self.sale_order.id], state='done')
        with mute_logger('tcrm.addons.sale.models.payment_transaction'):
            tx._post_process()
    ...
    def test_invoice_is_final(self):
        ...
        with mute_logger('tcrm.addons.sale.models.payment_transaction'), patch(
            'tcrm.addons.sale.models.sale_order.SaleOrder._create_invoices',
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        ...
        with self.assertRaises(JsonRpcException, msg='tcrm.exceptions.ValidationError'):
    ...
    def test_partial_payment_confirm_order(self):
        ...
        with patch(
            'tcrm.addons.sale.models.sale_order.SaleOrder._send_order_notification_mail',
