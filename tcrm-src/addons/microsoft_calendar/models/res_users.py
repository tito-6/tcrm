...
import logging
import requests
from tcrm.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
from datetime import datetime, timedelta

from tcrm import api, fields, models, _, Command
from tcrm.exceptions import UserError
from tcrm.loglevels import exception_to_unicode
from tcrm.addons.microsoft_account.models import microsoft_service
from tcrm.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class ResUsers(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Tcrm
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2tcrm(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Tcrm -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_tcrm2microsoft()
        ...
        (events - synced_events)._sync_tcrm2microsoft()
        self.sudo().microsoft_last_sync_date = datetime.now()
    ...
    def _set_ICP_first_synchronization_date(self, now):
        ...
        created Tcrm events and thus avoid spamming invitations for those events.
        """
