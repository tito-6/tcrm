...
from tcrm.sql_db import BaseCursor
...
from tcrm.addons.microsoft_account.models.microsoft_service import TIMEOUT
...
# This ensures changes are sent to Microsoft only if they really happened in the Tcrm database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in Tcrm.
...
class MicrosoftCalendarSync(models.AbstractModel):
    ...
    def _sync_recurrence_microsoft2tcrm(self, microsoft_events, new_events=None):
        ...
        for recurrent_master in recurrent_masters:
            new_calendar_recurrence = dict(
                self.env['calendar.recurrence']._microsoft_to_tcrm_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            ...
            base_values = dict(
                self.env['calendar.event']._microsoft_to_tcrm_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            to_create_values = []
            ...
            for recurrent_event in to_create:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_tcrm_recurrence_values(recurrent_event, base_values)
                else:
                    value = self.env['calendar.event']._microsoft_to_tcrm_values(recurrent_event, default_values)
            ...
            new_recurrence_tcrm = self.env['calendar.recurrence'].with_context(dont_notify=True).create(new_calendar_recurrence)
            new_recurrence_tcrm.base_event_id = new_recurrence_tcrm.calendar_event_ids[0] if new_recurrence_tcrm.calendar_event_ids else False
            new_recurrence |= new_recurrence_tcrm
        ...
        for recurrent_master_id in ms_recurrence_ids:
            ...
            for recurrent_event in to_update:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_tcrm_recurrence_values(
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        """
        Update Tcrm events from Outlook recurrence and events.
        """
        ...
        for e in events_to_update:
            if e.type == "exception":
                event_values = self.env['calendar.event']._microsoft_to_tcrm_values(e)
            elif e.type == "occurrence":
                event_values = self.env['calendar.event']._microsoft_to_tcrm_recurrence_values(e)
            else:
                event_values = None
            ...
            if event_values:
                ...
                tcrm_event = self.env['calendar.event'].browse(e.tcrm_id(self.env)).exists().with_context(
                    no_mail_to_attendees=True, mail_create_nolog=True
                )
                tcrm_event.with_context(dont_notify=True).write(dict(event_values, need_sync_m=False))
                update_events |= tcrm_event
    ...
    @api.model
    def _sync_microsoft2tcrm(self, microsoft_events: MicrosoftEvent):
        """
        Synchronize Microsoft recurrences in Tcrm.
        Creates new recurrences, updates existing ones.
        :return: synchronized tcrm
        """
        existing = microsoft_events.match_with_tcrm_events(self.env)
        ...
        tcrm_values = [
            dict(self._microsoft_to_tcrm_values(e, with_ids=True), need_sync_m=False)
            for e in (new - new_recurrence)
        ]
        synced_events = self.with_context(dont_notify=True, skip_contact_description=True)._create_from_microsoft(new, tcrm_values)
        synced_recurrences, updated_events = self._sync_recurrence_microsoft2tcrm(existing, new_recurrence)
        ...
        # Get sync lower bound days range for checking if old events must be updated in Tcrm.
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            # Last updated wins.
            # This could be dangerous if microsoft server time and tcrm server time are different
            if mevent.is_recurrence():
                tcrm_event = self.env['calendar.recurrence'].browse(mevent.tcrm_id(self.env)).exists()
            else:
                tcrm_event = self.browse(mevent.tcrm_id(self.env)).exists()

            if tcrm_event:
                tcrm_event_updated_time = pytz.utc.localize(tcrm_event.write_date)
                ...
                if lower_bound_day_range:
                    update_time_diff = ms_event_updated_time - tcrm_event_updated_time
                    old_event_update_condition = tcrm_event._check_old_event_update_required(int(lower_bound_day_range), update_time_diff)

                if ms_event_updated_time >= tcrm_event_updated_time and old_event_update_condition:
                    vals = dict(tcrm_event._microsoft_to_tcrm_values(mevent), need_sync_m=False)
                    tcrm_event.with_context(dont_notify=True)._write_from_microsoft(mevent, vals)

                    if tcrm_event._name == 'calendar.recurrence':
                        update_events = tcrm_event._update_microsoft_recurrence(mevent, microsoft_events)
                        synced_recurrences |= tcrm_event
                        synced_events |= update_events
                    else:
                        synced_events |= tcrm_event

        return synced_events, synced_recurrences

    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in Tcrm should be updated locally. This verification is necessary because
        sometimes events in Tcrm have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_delete(self, user_id, event_id, timeout=TIMEOUT):
        """
        Once the event has been really removed from the Tcrm database, remove it from the Outlook calendar.
    ...
    def _microsoft_patch(self, user_id, event_id, values, timeout=TIMEOUT):
        """
        Once the event has been really modified in the Tcrm database, modify it in the Outlook calendar.
    ...
    def _microsoft_insert(self, values, timeout=TIMEOUT):
        """
        Once the event has been really added in the Tcrm database, add it in the Outlook calendar.
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and Tcrm currently
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """
        Return records that should be synced from tcrm to Microsoft
    ...
    @api.model
    def _microsoft_to_tcrm_values(
        self, microsoft_event: MicrosoftEvent, default_reminders=(), default_values=None, with_ids=False
    ):
        """
        Implements this method to return a dict of Tcrm values corresponding
        to the Microsoft event given as parameter
        :return: dict of Tcrm formatted values
        """
    ...
    def _is_microsoft_insertion_blocked(self, sender_user):
        ...
        This is a necessary step for ensuring data match between Tcrm and Microsoft,
