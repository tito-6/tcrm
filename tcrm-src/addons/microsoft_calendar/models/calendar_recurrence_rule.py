...
class CalendarRecurrence(models.Model):
    ...
    def _inverse_rrule(self):
        ...
        # modified in Tcrm but computed from other fields).
    ...
    def _write_from_microsoft(self, microsoft_event, vals):
        ...
        # event_tz is written on event in Microsoft but on recurrence in Tcrm
        vals['event_tz'] = microsoft_event.start.get('timeZone')
        super()._write_from_microsoft(microsoft_event, vals)
        new_event_values = self.env["calendar.event"]._microsoft_to_tcrm_values(microsoft_event)
    ...
    def _get_microsoft_sync_domain(self):
        # Do not sync Tcrm recurrences with Outlook Calendar anymore.
    ...
    @api.model
    def _microsoft_to_tcrm_values(self, microsoft_recurrence, default_reminders=(), default_values=None, with_ids=False):
