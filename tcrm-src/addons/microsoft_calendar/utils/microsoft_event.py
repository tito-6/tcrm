...
from tcrm.tools.misc import ReadonlyDict


class MicrosoftEvent(abc.Set):
    """
    This helper class holds the values of a Microsoft event.
    Inspired by Tcrm recordset, one instance can be a single Microsoft event or a
    ...
    def tcrm_id(self, env):
        return self._tcrm_id

    def _meta_tcrm_id(self, microsoft_guid):
        """Returns the Tcrm id stored in the Microsoft Event metadata.
    ...
    def tcrm_ids(self):
        """
        Get the list of Tcrm event ids already mapped with Outlook events (self)
        """
        return tuple(e._tcrm_id for e in self if e._tcrm_id)

    def _load_tcrm_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing Tcrm events:
        1) extract unmapped events only,
        2) match Tcrm events and Outlook events which have both a ICalUId set,
        3) match remaining events,
        Returns the list of mapped events
        """
        mapped_events = [e.id for e in self if e._tcrm_id]
        ...
        tcrm_events = model_env.with_context(active_test=False).search([
            '|',
            ('ms_universal_event_id', "in", unmapped_events.uids),
            ('microsoft_id', "in", unmapped_events.ids)
        ]).with_env(env)

        # 1. try to match unmapped events with Tcrm events using their iCalUId
        unmapped_events_with_uids = unmapped_events.filter(lambda e: e.iCalUId)
        tcrm_events_with_uids = tcrm_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in tcrm_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            tcrm_id = mapping.get(ms_event.iCalUId)
            if tcrm_id:
                ms_event._events[ms_event.id]['_tcrm_id'] = tcrm_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with Tcrm events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.microsoft_id: e for e in tcrm_events}

        for ms_event in unmapped_events:
            tcrm_event = mapping.get(ms_event.id)
            if tcrm_event:
                ms_event._events[ms_event.id]['_tcrm_id'] = tcrm_event.id
                mapped_events.append(ms_event.id)

                # don't forget to also set the global event ID on the Tcrm event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current Tcrm user is the organizer of the event according to Outlook event, so return his id.
        2) the current Tcrm user is NOT the organizer and:
           2.1) we are able to find a Tcrm user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a Tcrm user matching the organizer email address and we return False, meaning
                that no Tcrm user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        ...
        if organizer_email:
            # Warning: In Microsoft: 1 email = 1 user; but in Tcrm several users might have the same email
    ...
    def match_with_tcrm_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing Tcrm events, and return the list of matched events
        """
        ...
        mapped_recurrences = recurrence_candidates._load_tcrm_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_tcrm_ids_from_db(env)
