...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Tcrm event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Tcrm events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Tcrm events
        # -----------------------------------------------------------------------------------------

        self.expected_tcrm_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_tcrm_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_tcrm_recurrency_events_from_outlook = [
            {
    ...
    def sync_tcrm_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Tcrm. A trick for deactivating the checking
    ...
    def assert_tcrm_event(self, tcrm_event, expected_values):
        """
        Assert that an Tcrm event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        tcrm_event_values = tcrm_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(tcrm_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, tcrm_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_tcrm_recurrence(self, tcrm_recurrence, expected_values):
        """
        Assert that an Tcrm recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        tcrm_recurrence_values = tcrm_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, tcrm_recurrence_values.get(k), msg=f"'{k}' mismatch")
