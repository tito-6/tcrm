...
from tcrm.addons.microsoft_calendar.tests.common import TestCommon


class TestSynctcrm2MicrosoftMail(TestCommon, MailCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
    ...
    def test_change_organizer_pure_tcrm_event(self):
        """
        Test that changing organizer on a pure Tcrm event (not synced with Microsoft)
        ...
        self.assertTrue(event.active, "Pure Tcrm event should not be archived when changing organizer")
