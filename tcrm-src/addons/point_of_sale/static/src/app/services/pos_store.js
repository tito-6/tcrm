...
import { markRaw, reactive } from "@tcrm/owl";
...
export class PosStore extends WithLazyGetterTrap {
    ...
    get session() {
        return this.data.models["pos.session"].get(tcrm.pos_session_id);
    }
    ...
    async loadNewProducts(domain, offset = 0, limit = 0) {
        const result = await this.data.callRelated(
            ...
            [tcrm.pos_config_id, domain, offset, limit],
            {},
            false
        );
        this.productAttributesExclusion = this.computeProductAttributesExclusion(
    ...
    redirectToBackend() {
        window.location = "/tcrm/action-point_of_sale.action_client_pos_menu";
    }
