...
export class PosData extends Reactive {
    ...
    async checkConnectivity() {
        try {
            ...
            // if the Tcrm server is still accessible.
            //
            // This method also makes it possible to run local tests when no connection is
            // available and an Tcrm server is running locally.
            //
    ...
    initializeWebsocket() {
        this.onNotified = getOnNotified(this.bus, tcrm.access_token);
    }
    ...
    get databaseName() {
        return `point-of-sale-${tcrm.pos_config_id}-${tcrm.info?.db}`;
    }
        ...
        if (
            ...
            try {
                ...
                const data = await this.orm.call(
                    "pos.session",
                    "load_data",
                    [tcrm.pos_session_id, PosData.modelToLoad],
                    {
                        context: {
    ...
    async loadFieldsAndRelations() {
        const key = `pos_data_params_${tcrm.pos_config_id}`;
            ...
            try {
                if (["product.product", "product.template"].includes(model)) {
                    ...
                    await this.callRelated(
                        ...
                        [tcrm.pos_config_id, [[domain, "in", Array.from(ids)]], 0, 0],
                        {
                            context: {
                                load_archived: true,
                            },
    ...
    async loadServerOrders(domain) {
        ...
        const config = this.models["pos.config"].get(tcrm.pos_config_id);
        const session = this.models["pos.session"].get(tcrm.pos_session_id);
