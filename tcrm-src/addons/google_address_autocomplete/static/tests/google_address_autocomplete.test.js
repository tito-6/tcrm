...
import { runAllTimers } from "@tcrm/hoot-mock";
import {
    contains,
    defineModels,
    fields,
...
test("correctly fill all standard fields", async () => {
    ...
    currentInput = "tcrm farm 2";
    await contains(".o_field_widget[name='street'] input").edit("tcrm farm 2", { confirm: false });
    await runAllTimers();
    ...
    currentInput = "tcrm farm 3";
    await contains(".o_field_widget[name='street'] input").edit("tcrm farm 3", { confirm: false });
    await runAllTimers();
    expect.verifySteps(["/autocomplete/address"]);
...
test("fills current field with values of unknown ones", async () => {
    ...
    await contains(".o_field_widget[name='some_char'] input").edit("tcrm farm 2", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
...
test("typing in input should make form dirty", async () => {
    ...
    expect.verifySteps([{ street: "tcrm farm 3" }]);
});
