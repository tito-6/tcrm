...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('tcrm-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(tcrmPartnerData) {
        // Create the markers and cluster them on the map
        if (tcrmPartnerData) { /* tcrm_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < tcrm_partner_data.counter; i++) {
                const prom = set_marker(tcrm_partner_data.partners[i])
    ...
    initializeMarkers(tcrm_partner_data);
