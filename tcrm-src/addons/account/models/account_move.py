...
from tcrm import api, fields, models, _, modules
from tcrm.tools.sql import column_exists, create_column
from tcrm.addons.account.tools import format_structured_reference_iso
from tcrm.exceptions import UserError, ValidationError, AccessError, RedirectWarning
from tcrm.fields import Command, Domain
...
from tcrm.tools.mail import email_re, email_split, is_html_empty, generate_tracking_message_id
from tcrm.tools.misc import StackMap
...
class AccountMove(models.Model):
    ...
    quick_edit_total_amount = fields.Monetary(
        ...
        help='Use this field to encode the total amount of the invoice.\n'
             'tcrm will automatically create one invoice line with default values to match it.',
    )
    ...
    def _get_invoice_reference_number_invoice(self):
        ...
        """
        ref = self._get_invoice_reference_tcrm_invoice() or ''
    ...
    def _get_invoice_reference_number_partner(self):
        ...
        """
        ref = self._get_invoice_reference_tcrm_partner()
        return ''.join(char for char in ref if char.isdigit())

    def _get_invoice_reference_tcrm_invoice(self):
        """ This computes the reference based on the tcrm format.
    ...
    def _get_invoice_reference_tcrm_partner(self):
        """ This computes the reference based on the tcrm format.
            The data used is the reference set on the partner or its database
            id otherwise. For instance if the reference of the customer is
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
        ...
        values.update({
            ...
            'invoice_user_id': self.invoice_user_id.id,  # otherwise user would be tcrmBot
        })
    ...
    def _message_post_after_hook(self, new_message, message_values):
        ...
        if self.env.context.get('from_alias'):
            ...
            # Only trigger decoding if the message was sent by an active internal user (note tcrmBot is always inactive).
