...
from tcrm.tests.common import tagged, new_test_user, TransactionCase
from tcrm.tools import mute_logger

from tcrm.addons.base.tests.common import HttpCase
from tcrm.addons.crm.tests.common import TestCrmCommon
from tcrm.addons.mail.controllers.thread import ThreadController
...
from tcrm.addons.website_crm_partner_assign.controllers.main import (
    WebsiteAccount,
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('tcrm.addons.base_geolocalize.models.base_geocoder.BaseGeocoder.geo_find', wraps=geo_find)
...
class TestPartnerLeadPortal(TestCrmCommon):
    ...
    @patch('tcrm.http.GeoIP')
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        """
            This test checks situation when "{tcrmURL}/partners" is visited from foreign country without resellers.
        ...
        """
        # Patch GeoIp so it acts, as if Tcrm client is located in Mexico
