...
OutFile "${TOOLSDIR}\server\tcrm_iot_setup_${VERSION}.exe"
...
; English
LangString DESC_tcrm_IoT ${LANG_ENGLISH} "Install the tcrm Server with IoT modules."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact tcrm for Partnership and/or Support"
LangString TITLE_tcrm_IoT ${LANG_ENGLISH} "tcrm IoT"
...
; French
LangString DESC_tcrm_IoT ${LANG_FRENCH} "Installation du Serveur tcrm avec les modules IoT."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez tcrm pour un Partenariat et/ou du Support"
LangString TITLE_tcrm_IoT ${LANG_FRENCH} "tcrm IoT"
...
Section $(TITLE_tcrm_IoT) Sectiontcrm_IoT
    ...
    # Cloning tcrm
    DetailPrint "Cloning tcrm"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" clone --filter=tree:0 -b 19.0 --single-branch --no-checkout https://github.com/tcrm/tcrm.git "$INSTDIR\tcrm"'

    DetailPrint "Configuring Sparse Checkout for IoT modules"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\tcrm" sparse-checkout init --no-cone'
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\tcrm" sparse-checkout set addons/web addons/hw_* addons/iot_drivers addons/iot_base addons/iot_box_image/configuration addons/point_of_sale/tools/posbox/configuration/requirements.txt tcrm tcrm-bin'

    DetailPrint "Checking out the repository"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\tcrm" checkout'
    ...
    DetailPrint "Configuring $(TITLE_tcrm_IoT)"
    # Set data_dir
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "data_dir" "$INSTDIR\sessions"
    # set modules to load
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "server_wide_modules" "iot_drivers,web"
    # Configure logging
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "logfile" "$INSTDIR\tcrm.log"
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "log_handler" ":WARNING"
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "log_level" "warn"
    # Other configuration
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\tcrm.conf" "options" "max_cron_threads" "0"

    DetailPrint "Installing Windows service"
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\tcrm\tcrm-bin\" -c "\"$INSTDIR\tcrm.conf\"'
...
Section -$(TITLE_Nginx) Nginx
    ...
    File "..\..\tcrm\addons\iot_box_image\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\tcrm\addons\iot_box_image\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectiontcrm_IoT} $(DESC_tcrm_IoT)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\tcrm IoT ${VERSION}"
