...
!ifndef SERVICENAME
	!define SERVICENAME 'tcrm-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\tcrmbuild'
!endif

!define PRODUCT_NAME "tcrm"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\tcrm Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\tcrm_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\tcrm-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\tcrm-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\tcrm-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\tcrm-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.tcrm.com/page/contactus"
...
; English
LangString DESC_tcrm_Server ${LANG_ENGLISH} "Install the tcrm Server with all the tcrm standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by tcrm."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact tcrm for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the tcrm Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "tcrm Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "tcrm Server Only"
LangString TITLE_tcrm_Server ${LANG_ENGLISH} "tcrm Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start tcrm"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_tcrm_Server ${LANG_FRENCH} "Installation du Serveur tcrm avec tous les modules tcrm standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par tcrm."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez tcrm pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur tcrm"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur tcrm Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur tcrm"
LangString TITLE_tcrm_Server ${LANG_FRENCH} "Serveur tcrm"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer tcrm"
...
Section $(TITLE_tcrm_Server) Sectiontcrm_Server
    SectionIn 1 2
    ...
    # If there is a previous install of the tcrm Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "addons_path" "$INSTDIR\server\tcrm\addons"
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\tcrm.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\tcrm-bin" --stop-after-init -c "$INSTDIR\server\tcrm.conf" --logfile "$INSTDIR\server\tcrm.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\tcrm-bin\" -c "\"$INSTDIR\server\tcrm.conf\"'

    Call RestarttcrmService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_tcrm" \
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@tcrm.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.tcrm.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@tcrm.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectiontcrm_Server} $(DESC_tcrm_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\tcrm ${VERSION}"
...
Function ComponentLeave
    SectionGetFlags ${Sectiontcrm_Server} $0
...
Function RestarttcrmService
    DetailPrint "Restarting tcrm Service"
