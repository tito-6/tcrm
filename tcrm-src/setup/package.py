...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'tcrm', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/tcrm && \
    groupadd -g %(group_id)s tcrm && \
    useradd -u %(user_id)s -g tcrm tcrm -d /var/lib/tcrm && \
    mkdir /data && \
    chown tcrm:tcrm /var/lib/tcrm /data
USER tcrm
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class tcrmTestTimeoutError(Exception):
    pass


class tcrmTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise tcrmTestError("Installation of package failed")
        else:
        ...
        raise tcrmTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/tcrm_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.tcrm_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'tcrm/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'tcrm-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER tcrm', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='tcrm', exposed_port=None, detach=False, timeout=None):
    ...
    def test_tcrm(self):
        logging.info('Starting to test tcrm install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)  # give some time for tcrm to install and start
            if os.path.exists(os.path.join(args.build_dir, 'tcrm.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise tcrmTestTimeoutError('tcrm pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise tcrmTestError('Error while installing/starting tcrm after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'tcrm-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/tcrm-*.tar.gz' % self.args.build_dir)[0], '%s/tcrm_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/tcrm-*.zip' % self.args.build_dir)[0], '%s/tcrm_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s tcrm"',
            'su tcrm -s /bin/bash -c "python3 -m venv /var/lib/tcrm/tcrmvenv"',
            'su tcrm -s /bin/bash -c "/var/lib/tcrm/tcrmvenv/bin/python3 -m pip install --upgrade pip"',
            'su tcrm -s /bin/bash -c "/var/lib/tcrm/tcrmvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su tcrm -s /bin/bash -c "/var/lib/tcrm/tcrmvenv/bin/python3 -m pip install /data/src/tcrm_{VERSION}.{TSTAMP}.tar.gz"',
            'su tcrm -s /bin/bash -c "createdb mycompany"',
            'su tcrm -s /bin/bash -c "/var/lib/tcrm/tcrmvenv/bin/tcrm -d mycompany -i base --stop-after-init"',
            'su tcrm -s /bin/bash -c "/var/lib/tcrm/tcrmvenv/bin/tcrm -d mycompany --pidfile=/data/src/tcrm.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'tcrm-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_tcrm()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/tcrm (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../tcrm_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'tcrm-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/tcrm_{VERSION}.{TSTAMP}_all.deb',
            'su tcrm -s /bin/bash -c "tcrm -d mycompany -i base --pidfile=/data/src/tcrm.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'tcrm-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_tcrm()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/tcrm/rpmbuild'
        build_date = datetime.now().strftime('%a %b %d %Y')
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/tcrm.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./tcrm-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/tcrm-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" --define "%release {TSTAMP}" --define "%build_date {build_date}" /data/src/setup/rpm/tcrm.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/tcrm*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'tcrm-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/tcrm-*.noarch.rpm' % self.args.build_dir)[0], '%s/tcrm_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s tcrm"',
            'su tcrm -c "createdb mycompany"',
            'dnf install -q /data/src/tcrm_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su tcrm -s /bin/bash -c "tcrm -c /etc/tcrm/tcrm.conf -d mycompany -i base --stop-after-init"',
            'su tcrm -s /bin/bash -c "tcrm -c /etc/tcrm/tcrm.conf -d mycompany --pidfile=/data/src/tcrm.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'tcrm-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_tcrm()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'tcrm-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\tcrmbuild\server\setup\win32\setup.nsi"
    ...
    def build(self):
        ...
        container_python = '/var/lib/tcrm/.wine/drive_c/tcrmbuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        self.run(' && '.join(cmds), self.args.build_dir, 'tcrm-win-build-%s' % TSTAMP)
...
class DockerIot(DockerWine):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\tcrmbuild\server\setup\win32\setup-iot.nsi"
        self.nt_service_name = "tcrm-iot"

    def build_image(self):
        shutil.copy(os.path.join(self.args.build_dir, 'tcrm/addons/iot_box_image/configuration/requirements.txt'), self.docker_dir / 'requirements-iot.txt')
