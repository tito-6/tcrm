...
class MarketingHub(models.AbstractModel):
    ...
    def _campaign_domain(self, company, selected=None, date_from=None, date_to=None):
        ...
        dt_from = self._parse_date_bound(date_from, end=False)
        dt_to = self._parse_date_bound(date_to, end=True)
        # Campaign metrics are period snapshots; filter by last sync / write as best proxy
        if dt_from:
            domain.append(('last_sync_at', '>=', fields.Datetime.to_string(dt_from)))
        if dt_to:
            domain.append(('last_sync_at', '<=', fields.Datetime.to_string(dt_to)))
        return domain
    ...
    def get_dashboard(self, selected_ad_account_id=None, date_from=None, date_to=None, lead_source=None):
        ...
        if not selected and ad_accounts:
            selected = ad_accounts.filtered(
                lambda a: 'model' in (a.name or '').lower() or 'inno' in (a.business_name or '').lower()
            )[:1] or ad_accounts[:1]
        ...
        # If date filter empties campaigns, fall back to account filter only (still show list)
        campaigns = self.env['tcrm.marketing.campaign'].search(campaign_domain, limit=120)
        if not campaigns and (date_from or date_to) and selected:
    ...
    def get_lead_charts(self, date_from=None, date_to=None, selected_ad_account_id=None):
        ...
        camp_domain = [('company_id', '=', company.id)]
        if selected_ad_account_id:
            camp_domain.append(('ad_account_id', '=', int(selected_ad_account_id)))
